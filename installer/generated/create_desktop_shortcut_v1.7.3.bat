@echo off
REM ========================================
REM WhisperJAV v1.7.3 Desktop Shortcut Creator
REM ========================================
REM
...
if %ERRORLEVEL% EQU 0 (
    ...
    if "%USE_LAUNCHER_EXE%"=="1" (
        ...
        echo.
        powershell -NoProfile -ExecutionPolicy Bypass -Command "$WshShell = New-Object -ComObject WScript.Shell; $Desktop = [Environment]::GetFolderPath('Desktop'); $ShortcutPath = Join-Path $Desktop '%SHORTCUT_NAME%'; $Shortcut = $WshShell.CreateShortcut($ShortcutPath); $Shortcut.TargetPath = '%TARGET_EXE%'; $Shortcut.WorkingDirectory = '%SCRIPT_DIR%'; if ('%ICON%' -ne '') { $Shortcut.IconLocation = '%ICON%' }; $Shortcut.Description = 'WhisperJAV v1.7.3 Japanese AV Subtitle Generator'; $Shortcut.Save()"
    ) else (
        REM Fallback: Point to pythonw.exe with module arguments
        echo Shortcut target: %PYTHONW% %TARGET_ARGS%
        echo.
        powershell -NoProfile -ExecutionPolicy Bypass -Command "$WshShell = New-Object -ComObject WScript.Shell; $Desktop = [Environment]::GetFolderPath('Desktop'); $ShortcutPath = Join-Path $Desktop '%SHORTCUT_NAME%'; $Shortcut = $WshShell.CreateShortcut($ShortcutPath); $Shortcut.TargetPath = '%PYTHONW%'; $Shortcut.Arguments = '%TARGET_ARGS%'; $Shortcut.WorkingDirectory = '%SCRIPT_DIR%'; if ('%ICON%' -ne '') { $Shortcut.IconLocation = '%ICON%' }; $Shortcut.Description = 'WhisperJAV v1.7.3 Japanese AV Subtitle Generator'; $Shortcut.Save()"
    )
) else (
    ...
    if "%USE_LAUNCHER_EXE%"=="1" (
        ...
        (
            ...
            echo Shortcut.Description = "WhisperJAV v1.7.3 - Japanese AV Subtitle Generator"
        ) > "%VBS_SCRIPT%"

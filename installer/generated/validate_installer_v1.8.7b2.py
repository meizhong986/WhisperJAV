...
def validate_yaml_syntax():
    ...
    try:
        ...
        # Check extra_files exist (except deferred downloads)
        # uv.exe is downloaded during build_installer.bat Phase 3, not during generation
        deferred_files = {'uv.exe'}
        missing_files = [
            f for f in data['extra_files']
            if not check_file_exists(f) and f not in deferred_files
        ]
        if missing_files:
            print_check(
                f"{filename} syntax",
        ...
        # Note deferred files that will be downloaded during build
        deferred_present = [f for f in data['extra_files'] if f in deferred_files]
        if deferred_present:
            print(f"          Note: {', '.join(deferred_present)} will be downloaded during build")

"""
WhisperJAV v1.8.7 Post-Install Script
======================================
...
4. Installs all Python dependencies from requirements_v1.8.7.txt
5. Installs WhisperJAV from GitHub
6. Creates desktop shortcut
7. Provides detailed installation summary

All output is logged to install_log_v1.8.7.txt
"""
...
LOG_FILE = os.path.join(sys.prefix, "install_log_v1.8.7.txt")

# ==============================================================================
# GPU Package Lock-in (prevents pip/uv from replacing GPU wheels with CPU wheels)
# ==============================================================================
# After installing PyTorch from the CUDA index, we record exact versions in a
# constraints file. All subsequent pip/uv installs use --constraint to prevent
# packages like openai-whisper, transformers, or accelerate from pulling in a
# different (CPU-only) version of torch from PyPI.
#
# This also protects llama-cpp-python CUDA wheels from being overwritten.
#
# See: https://pip.pypa.io/en/stable/user_guide/#constraints-files
GPU_CONSTRAINTS_FILE = os.path.join(sys.prefix, "gpu_constraints.txt")

# Whether constraints have been written (gate for --constraint injection)
_gpu_constraints_active = False


def _strip_local_version(version: str) -> str:
    """
    Strip PEP 440 local version suffix (e.g. +cpu, +cu128) from version string.

    PyTorch encodes accelerator in local version: 2.10.0+cpu, 2.10.0+cu128.
    uv's resolver cannot find these local versions on PyPI indexes, so
    constraint files must use the base version (2.10.0) instead.

    PEP 440 guarantees that ==2.10.0 matches 2.10.0+cpu and 2.10.0+cu128,
    so the constraint still prevents version changes.
    """
    return version.split('+')[0] if '+' in version else version


def create_gpu_constraints(packages: dict) -> bool:
    """
    Write a constraints file pinning GPU-specific packages.

    This prevents subsequent pip/uv installs from replacing GPU wheels
    with CPU-only versions from default PyPI.

    IMPORTANT: Local version suffixes (+cpu, +cu128) are stripped because
    uv cannot resolve them from PyPI. PEP 440 ensures ==2.10.0 still
    matches the installed 2.10.0+cu128. See install_log_v1.8.7b1.txt.

    Args:
        packages: Dict of {package_name: version_string}
                  e.g. {"torch": "2.10.0+cu128", "torchaudio": "2.10.0+cu128"}

    Returns:
        True if constraints file was written successfully
    """
    global _gpu_constraints_active

    try:
        lines = [
            "# Auto-generated by post_install - DO NOT EDIT",
            "# Pins GPU packages to prevent pip/uv from replacing with CPU versions",
            "# Note: local version suffixes (+cpu/+cu*) stripped for uv compatibility",
            "",
        ]
        for pkg, ver in packages.items():
            base_ver = _strip_local_version(ver)
            lines.append(f"{pkg}=={base_ver}")

        with open(GPU_CONSTRAINTS_FILE, "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")

        _gpu_constraints_active = True
        log(f"  GPU lock-in: pinned {list(packages.keys())} in {GPU_CONSTRAINTS_FILE}")
        return True
    except Exception as e:
        log(f"  WARNING: Could not write GPU constraints file: {e}")
        return False


def append_gpu_constraint(package: str, version: str) -> bool:
    """
    Append a package pin to the existing GPU constraints file.

    Used after installing llama-cpp-python to protect it from overwrite.

    Args:
        package: Package name (e.g., "llama-cpp-python")
        version: Exact version string (e.g., "0.3.21")
    """
    global _gpu_constraints_active

    try:
        mode = "a" if os.path.exists(GPU_CONSTRAINTS_FILE) else "w"
        with open(GPU_CONSTRAINTS_FILE, mode, encoding="utf-8") as f:
            if mode == "w":
                f.write("# Auto-generated by post_install - DO NOT EDIT\n\n")
            base_ver = _strip_local_version(version)
            f.write(f"{package}=={base_ver}\n")

        _gpu_constraints_active = True
        log(f"  GPU lock-in: added {package}=={_strip_local_version(version)}")
        return True
    except Exception as e:
        log(f"  WARNING: Could not append to GPU constraints: {e}")
        return False
...
def log_file_only(message: str):
    """Log message to file only (not console). Used for verbose output."""
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    line = f"[{ts}] {message}"
    try:
        with open(LOG_FILE, "a", encoding="utf-8") as f:
...
def verify_existing_torch_stack() -> Tuple[bool, Optional[str]]:
    """
    Check if CUDA-enabled torch and torchaudio meet requirements.

    Returns:
        (valid, error) — valid=True means torch is good AND constraints written.
        error is a failure reason string if something went wrong, None otherwise.
        When error is set, the caller should abort (not retry with fresh install).
    """
    ...
    if cuda_tuple >= MIN_TORCH_CUDA_VERSION:
        ...
        # Create GPU constraints to protect pre-existing torch from overwrites.
        # If this fails, abort — continuing without protection risks silent
        # CUDA→CPU replacement in Phase 3.5/4 (much harder to debug).
        gpu_pins = {"torch": torch.__version__}
        try:
            gpu_pins["torchaudio"] = torchaudio.__version__
        except Exception:
            pass
        try:
            import torchvision
            gpu_pins["torchvision"] = torchvision.__version__
        except Exception:
            pass
        if not create_gpu_constraints(gpu_pins):
            reason = f"Cannot write GPU constraints file: {GPU_CONSTRAINTS_FILE} (check permissions/disk space)"
            log(f"FATAL: {reason}")
            log("       Without this file, subsequent installs may replace CUDA PyTorch with CPU-only.")
            return False, reason
        return True, None
...
def select_torch_install_plan(driver_info: DriverInfo) -> TorchInstallPlan:
        ...
        for entry in TORCH_DRIVER_MATRIX:
            if driver_info.driver_version >= entry.min_driver:
                return TorchInstallPlan(
                    ...
                    description=f"Install PyTorch/Torchaudio/Torchvision ({entry.cuda_version})",
                    uses_gpu=True,
    ...
    return TorchInstallPlan(
        ...
        description="Install PyTorch/Torchaudio/Torchvision (CPU-only)",
        uses_gpu=False,
        target_label="CPU",
...
def install_pytorch_and_torchaudio(plan: TorchInstallPlan) -> bool:
    ...
    log(f"Installing via {installer_label}: {' '.join(plan.pip_args)}")
    # use_constraints=False: PyTorch IS the GPU package — constraints don't exist yet
    return run_pip(plan.pip_args, plan.description, use_constraints=False)
...
def check_ffmpeg() -> bool:
    ...
    log("  ⚠ FFmpeg: NOT FOUND on system PATH")
    log("    WhisperJAV requires FFmpeg for audio/video processing.")
    log("    Install from: https://ffmpeg.org/download.html")
    log("    Or via conda: conda install -c conda-forge ffmpeg")
    return False
...
def install_pytorch(driver_info: Optional[DriverInfo] = None) -> Optional[str]:
    """
    Install PyTorch/Torchaudio with best available acceleration.

    Returns:
        None on success, or a failure reason string for diagnostics.
    """
    log_section("PyTorch Installation")

    existing_ok, existing_error = verify_existing_torch_stack()
    if existing_ok:
        return None
    if existing_error:
        # Torch exists and is valid, but constraints couldn't be written.
        # Don't waste time reinstalling — the filesystem problem won't fix itself.
    ...
    if plan.uses_gpu:
        ...
        if response != 'y':
            ...
            return "User declined CPU-only PyTorch"

    if not install_pytorch_and_torchaudio(plan):
        return "PyTorch package installation failed (pip/uv error)"
    ...
    try:
        ...
        # Lock GPU packages: pin torch/torchaudio to prevent subsequent pip installs
        # from replacing CUDA wheels with CPU-only versions from PyPI.
        # If this fails, abort — continuing without protection risks silent
        # CUDA→CPU replacement in Phase 3.5/4 (much harder to debug).
        gpu_pins = {"torch": torch.__version__}
        try:
        ...
        try:
            ...
            pass
        if not create_gpu_constraints(gpu_pins):
            log("FATAL: Cannot write GPU constraints file.")
            log(f"       Check write permissions to: {GPU_CONSTRAINTS_FILE}")
            log("       Without this file, subsequent installs may replace CUDA PyTorch with CPU-only.")
            return f"Cannot write GPU constraints file: {GPU_CONSTRAINTS_FILE} (check permissions/disk space)"

    except Exception as exc:
        log(f"FATAL: PyTorch installed but verification/pinning failed: {exc}")
        log("       Cannot guarantee GPU package protection. Aborting.")
        return f"PyTorch verification/pinning failed: {exc}"
...
def is_git_network_error(error_output: str) -> bool:
    """
    Detect if error was caused by Git network failure (timeout, DNS, proxy).

    Common scenarios:
    - 21-second TCP timeout: Windows default when GFW blocks GitHub
    - DNS resolution failure: bundled git doesn't inherit system proxy
    - Connection refused/reset: firewall or proxy issues

    See: https://github.com/meizhong986/WhisperJAV/issues/111
    See: https://github.com/meizhong986/WhisperJAV/issues/210
    """
    network_patterns = [
    ...
    return any(pattern.lower() in error_lower for pattern in network_patterns)


def configure_git_for_slow_connections() -> bool:
    """
    Configure Git with extended timeouts and proxy for users behind GFW.

    This addresses two distinct failure modes:
    1. 21-second TCP timeout: Windows default when GFW blocks/slows GitHub
    2. DNS resolution failure: bundled git doesn't inherit system proxy
    ...
    # Also detect and propagate system proxy (fixes DNS resolution failures)
    log("Checking for system proxy settings...")
...
def detect_and_configure_proxy() -> bool:
    """
    Detect system proxy settings and propagate to environment and bundled git.

    Chinese users (GFW) typically access GitHub via VPN/proxy configured at the
    OS level. The conda-bundled git doesn't inherit these settings, causing DNS
    resolution failures even when the system browser/git works fine.
    ...
    # 1. Check if proxy already in environment
    for var in ['HTTPS_PROXY', 'HTTP_PROXY', 'https_proxy', 'http_proxy']:
        proxy = os.environ.get(var, "").strip()
        if proxy:
            log(f"  Proxy already set: {var}={proxy}")
            # Ensure both upper and lower case are set for compatibility
            os.environ['HTTP_PROXY'] = proxy
            os.environ['HTTPS_PROXY'] = proxy
            os.environ['http_proxy'] = proxy
            os.environ['https_proxy'] = proxy
            _configure_git_proxy(proxy)
            _proxy_configured = True
            return True

    # 2. Check Windows registry for system proxy (IE/Edge settings)
    try:
        import winreg
        key = winreg.OpenKey(
        ...
        if proxy_enable:
            ...
            if proxy_server:
                ...
                os.environ['http_proxy'] = proxy_server
                os.environ['https_proxy'] = proxy_server
                log(f"  Windows system proxy detected: {proxy_server}")
                _configure_git_proxy(proxy_server)
                _proxy_configured = True
                return True
        winreg.CloseKey(key)
    except Exception:
    ...
    log("  No system proxy detected.")
    _proxy_configured = True  # Don't retry detection
    return False


def _configure_git_proxy(proxy_url: str) -> None:
    """Configure bundled git to use the detected proxy."""
    git_exe = shutil.which("git")
    if not git_exe:
        bundled = os.path.join(sys.prefix, "Library", "bin", "git.exe")
    ...
    try:
        ...
        log(f"  Configured git proxy: {proxy_url}")
    except Exception as e:
        log(f"  WARNING: Could not configure git proxy: {e}")
...
def run_pip(args: list, description: str, retries: int = 3,
            allow_fail: bool = False, use_constraints: bool = True) -> bool:
    """
    ...
    GPU Protection: When gpu_constraints.txt exists and use_constraints=True,
    all install commands automatically include --constraint to prevent pip/uv
    from replacing CUDA packages with CPU-only versions from PyPI.

    Args:
        args: Pip-style arguments (e.g., ["install", "package"])
        description: Human-readable description for logging
        retries: Number of retry attempts (default 3)
        allow_fail: If True, return False on failure instead of logging FATAL
        use_constraints: If True, inject --constraint gpu_constraints.txt
                         Set to False for PyTorch install itself (Phase 3)
    ...
    # Inject GPU constraints to prevent CUDA→CPU package replacement
    # Only for "install" commands when constraints file exists and is active
    is_install = args and args[0] == "install"
    if (use_constraints and _gpu_constraints_active and is_install
            and os.path.exists(GPU_CONSTRAINTS_FILE)
            and "--no-deps" not in args):
        normalized_args = ["-c", GPU_CONSTRAINTS_FILE] + normalized_args
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if result.stdout:
                ...
                for line in lines:
                    log_file_only(f"  {line}")
                # Show last 20 lines on console
                if len(lines) > 20:
                    log(f"  ... ({len(lines) - 20} lines written to log file only)")
            ...
            # Check for Git network errors (Issue #111 timeout, #210 DNS)
            if is_git_install and is_git_network_error(error_output):
                if not _git_timeouts_configured:
                    log("")
                    log("Detected Git network failure (common with VPN/GFW/proxy).")
                    configure_git_for_slow_connections()
                    # Don't count this as a failed attempt
                    log("Retrying with extended timeouts and proxy...")
                    time.sleep(5)
                    continue
    ...
    if allow_fail:
        log(f"WARNING: {description} failed after {retries} attempts (non-fatal)")
    else:
...
def create_failure_file(error_message: str):
    ...
    failure_file = os.path.join(sys.prefix, "INSTALLATION_FAILED_v1.8.7.txt")
    try:
        with open(failure_file, "w", encoding="utf-8") as f:
            f.write("WhisperJAV v1.8.7 Installation Failed\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Error: {error_message}\n\n")
            f.write("Manual Cleanup Steps:\n")
            f.write(f"1. Delete installation directory: {sys.prefix}\n")
            f.write("2. Delete desktop shortcut: WhisperJAV v1.8.7.lnk\n")
            f.write(f"3. Check install_log_v1.8.7.txt for details\n\n")
            f.write("Common Solutions:\n")
            f.write("- Out of disk space: Free up 8GB and retry\n")
            f.write("- Permission error: Run installer as administrator, or check antivirus\n")
            ...
            f.write(f"Full details: {LOG_FILE}\n\n")
...
def get_installation_size_gb() -> Optional[float]:
    """Calculate total installation size in GB."""
    try:
        total_size = 0
...
def get_gpu_vram_mb() -> Optional[Tuple[str, int]]:
    """Get GPU name and VRAM in MB via nvidia-smi."""
    try:
        result = subprocess.run(
...
def verify_import(module_name: str, display_name: str) -> Tuple[bool, Optional[str]]:
    """
    Try to import a module and return (success, version_or_error).

    Uses subprocess to avoid polluting the installer's own process with
    heavy imports (torch, whisper, etc. consume memory and may conflict).
    """
    python_exe = os.path.join(sys.prefix, 'python.exe')
    try:
        result = subprocess.run(
            [python_exe, "-c",
             f"import {module_name}; v = getattr({module_name}, '__version__', 'OK'); print(v)"],
            capture_output=True,
            text=True,
            timeout=30
        )
...
def run_post_install_verification() -> Tuple[int, int, list]:
    """
    Phase 6: Post-install verification.

    Checks that the installed application actually works by testing:
    1. Core whisperjav import
    2. Entry point executables exist
    3. Key dependency imports
    4. FFmpeg availability
    5. GPU VRAM info

    Returns:
        (pass_count, fail_count, warnings_list)
    """
    passes = 0
    fails = 0
    warnings = []

    # --- 6.1: WhisperJAV import smoke test ---
    log("")
    log("  6.1 WhisperJAV Import Smoke Test")
    log("  " + "-" * 40)

    ok, detail = verify_import("whisperjav", "WhisperJAV")
    if ok:
        log(f"  ✓ import whisperjav: {detail}")
        passes += 1
    else:
        log(f"  ✗ import whisperjav FAILED: {detail}")
        fails += 1

    # Test the main entry point import
    python_exe = os.path.join(sys.prefix, 'python.exe')
    try:
        result = subprocess.run(
            [python_exe, "-c", "from whisperjav.main import main; print('OK')"],
            capture_output=True, text=True, timeout=30
        )
        if result.returncode == 0:
            log("  ✓ from whisperjav.main import main: OK")
            passes += 1
        else:
            error = result.stderr.strip().split('\n')[-1] if result.stderr else "unknown"
            log(f"  ✗ from whisperjav.main import main FAILED: {error}")
            fails += 1
    except Exception as e:
        log(f"  ✗ from whisperjav.main import main FAILED: {e}")
        fails += 1

    # --- 6.2: Entry point executables ---
    log("")
    log("  6.2 Entry Point Executables")
    log("  " + "-" * 40)

    entry_points = [
        ("whisperjav.exe", True),
        ("whisperjav-gui.exe", True),
        ("whisperjav-translate.exe", False),
        ("whisperjav-upgrade.exe", False),
    ]
    for exe_name, is_critical in entry_points:
        exe_path = os.path.join(sys.prefix, "Scripts", exe_name)
        if os.path.exists(exe_path):
            size_kb = os.path.getsize(exe_path) // 1024
            log(f"  ✓ {exe_name} ({size_kb} KB)")
            passes += 1
        else:
            if is_critical:
                log(f"  ✗ {exe_name}: NOT FOUND (critical)")
                fails += 1
            else:
                log(f"  ⚠ {exe_name}: not found (optional)")
                warnings.append(f"{exe_name} not found")

    # Check --version on whisperjav.exe
    wj_exe = os.path.join(sys.prefix, "Scripts", "whisperjav.exe")
    if os.path.exists(wj_exe):
        try:
            result = subprocess.run(
                [wj_exe, "--version"],
            ...
            if result.returncode == 0:
                ...
                log(f"  ✓ whisperjav --version: {version_str}")
                passes += 1
            else:
                log(f"  ⚠ whisperjav --version: exit code {result.returncode}")
                warnings.append("whisperjav --version returned non-zero")
        except Exception as e:
            log(f"  ⚠ whisperjav --version: {e}")
            warnings.append(f"whisperjav --version: {e}")

    # --- 6.3: Key dependency imports ---
    log("")
    log("  6.3 Key Package Verification")
    log("  " + "-" * 40)
    ...
    for module, display in key_packages:
        ok, detail = verify_import(module, display)
        if ok:
            log(f"  ✓ {display}: {detail}")
            passes += 1
        else:
            log(f"  ✗ {display}: FAILED ({detail})")
            fails += 1
    ...
    for module, display in optional_packages:
        ...
        if ok:
            ...
            log(f"  ⚠ {display}: not available ({detail})")
            warnings.append(f"{display} not importable")
    ...
    if check_ffmpeg():
        ...
        warnings.append("FFmpeg not on PATH")

    # --- 6.5: GPU VRAM ---
    log("")
    log("  6.5 GPU Information")
    log("  " + "-" * 40)
    vram_info = get_gpu_vram_mb()
    if vram_info:
        gpu_name, vram_mb = vram_info
        log(f"  ✓ GPU: {gpu_name}")
        log(f"  ✓ VRAM: {vram_mb} MB ({vram_mb / 1024:.1f} GB)")
        if vram_mb < 4096:
            log("    ⚠ Low VRAM — large models may not fit. Consider using 'faster' mode.")
            warnings.append(f"Low VRAM ({vram_mb} MB)")
        passes += 1
    else:
        log("  - No NVIDIA GPU detected (or nvidia-smi unavailable)")

    # --- 6.6: Installation size ---
    log("")
    log("  6.6 Installation Size")
    log("  " + "-" * 40)
    install_size = get_installation_size_gb()
    if install_size is not None:
        log(f"  Installation size: {install_size:.1f} GB")
    else:
        log("  Could not determine installation size")

    return passes, fails, warnings


def print_installation_summary(install_start_time: float,
                               verification_results: Optional[Tuple[int, int, list]] = None):
    ...
    log("\n\n")
    log("=" * 80)
    log(" " * 20 + "WhisperJAV v1.8.7 Installation Complete!")
    log("=" * 80)
    log("")
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if vram_info:
                _, vram_mb = vram_info
                log(f"  ✓ GPU: {gpu_label} ({vram_mb / 1024:.1f} GB VRAM)")
            else:
                log(f"  ✓ GPU: {gpu_label}")
        else:
    ...
    if verification_results:
        passes, fails, warnings = verification_results
        if fails == 0:
            log(f"  ✓ Verification: {passes} checks passed")
        else:
            log(f"  ⚠ Verification: {passes} passed, {fails} FAILED")
        if warnings:
            for w in warnings:
                log(f"    ⚠ {w}")
...
def get_llama_cpp_source_info() -> Tuple[str, str, Optional[str], dict]:
    ...
    git_url = "llama-cpp-python @ git+https://github.com/JamePeng/llama-cpp-python.git"
    cmake_args = None
    env_vars = {}
...
def install_llama_cpp_python() -> bool:
    ...
    IMPORTANT: Server dependencies (uvicorn, fastapi, etc.) are already
    installed via requirements.txt [llm] extra in Phase 4. We do NOT run
    "pip install llama-cpp-python[server]" because that resolves from PyPI
    and can REPLACE the CUDA wheel with a CPU-only version.
    ...
    # Track which backend we installed (for validation reporting)
    ...
    if hf_wheel_path:
        ...
        if run_pip(
            ...
            installed_backend = hf_backend
            # Pin to prevent future overwrites
            _pin_llama_cpp_version()
            # Validate: pass the known backend — don't reverse-engineer from metadata
            valid, msg = validate_llama_cpp_installation(installed_backend)
            if valid:
    ...
    if wheel_url:
        ...
        if run_pip(
            ...
            installed_backend = wheel_backend
            _pin_llama_cpp_version()
            valid, msg = validate_llama_cpp_installation(installed_backend)
            if valid:
                log(f"  [OK] Validation passed: {msg}")
    ...
    # Server deps (uvicorn, fastapi, etc.) already installed via requirements.txt [llm] extra.
...
def _pin_llama_cpp_version():
    """Pin the installed llama-cpp-python version in GPU constraints file.

    This prevents any future pip/uv call from replacing the CUDA wheel
    ...
    try:
        ...
        if version:
            append_gpu_constraint("llama-cpp-python", version)
        else:
            log("  WARNING: Could not detect llama-cpp-python version — unable to pin")
    except Exception as e:
        log(f"  WARNING: Could not pin llama-cpp-python version: {e}")


def validate_llama_cpp_installation(expected_backend: str = None) -> Tuple[bool, str]:
    """
    ...
    The installer already knows which backend it installed (CUDA wheel, Metal,
    source build, etc.) and passes it via expected_backend. We trust this
    instead of trying to reverse-engineer the build type from package metadata,
    which is fragile (direct_url.json points to temp files, not original URLs).

    Args:
        expected_backend: The backend string from the installer (e.g.,
                          "CUDA (cu128 HuggingFace wheel v0.3.21)").
                          Used for reporting — the installer knows what it installed.
    ...
    # Step 3: Report GPU status based on what the installer knows it installed
    # The installer selected the wheel/source build — it knows the backend.
    # Trying to re-discover this from package metadata (direct_url.json) is
    # fragile: cached wheels have local temp paths, [server] extras may
    # overwrite metadata. Trust the installer's own knowledge instead.
    if expected_backend:
        is_gpu = any(tag in expected_backend.lower()
                     for tag in ["cuda", "metal", "gpu", "source"])
        if is_gpu:
            log(f"    [OK] GPU backend: {expected_backend}")
            return True, f"GPU: {expected_backend}"
        else:
            log(f"    [OK] Backend: {expected_backend}")
            return True, f"{expected_backend}"

    # Fallback: no expected_backend provided (shouldn't happen with new code)
    # Check if PyTorch has CUDA — if so, the wheel likely has CUDA too
    try:
        import torch
        if torch.cuda.is_available():
            log("    [OK] PyTorch CUDA available — GPU acceleration likely active")
            return True, "Import OK (PyTorch CUDA available)"
    except ImportError:
        pass
...
def embed_icon_in_exe(exe_path: str, icon_path: str) -> bool:
    ...
    """
    try:
        log("Embedding icon into executable...")

        # Try to import lief, install if not available
        try:
            import lief
        except ImportError:
            log("Installing LIEF library for icon embedding...")
            result = subprocess.run(
            ...
            if result.returncode != 0:
                log("INFO: Could not install LIEF library")
                return False
            import lief
        ...
        if not binary:
            log("INFO: Could not parse executable with LIEF")
            return False

        # Use the resources manager from the parsed binary (LIEF 0.15+ API)
        resources_manager = binary.resources_manager
        if resources_manager is None:
            log("INFO: Executable has no resources section — cannot embed icon")
            return False

        resources_manager.change_icon(icon_path)
        ...
        log("✓ Icon embedded successfully")
        return True

    except Exception as e:
        log(f"INFO: Could not embed icon: {e}")
        log("      The executable will use default Python icon")
        log("      The desktop shortcut will still display the correct icon")
        return False
...
def main() -> int:
    ...
    log_section("WhisperJAV v1.8.7 Post-Install Started")
    ...
    # Check FFmpeg (non-fatal warning — user can install later)
    check_ffmpeg()
    ...
    if pytorch_error:
        create_failure_file(f"PyTorch installation failed: {pytorch_error}")
        return 1
    ...
    # - --no-deps: All runtime dependencies are managed separately
    #   (torch from Phase 3, everything else from Phase 4 requirements.txt).
    #   This avoids dependency resolution entirely, which is both faster and
    #   eliminates any risk of uv replacing CUDA torch with CPU torch.
    #   PEP 517 build dependencies are still handled via build isolation.
    ...
    # Proactively detect and propagate system proxy for bundled git.
    # Chinese users often have proxies at OS level that bundled git can't see.
    detect_and_configure_proxy()

    for package_spec, description in git_packages:
        if not run_pip(
            ["install", "--no-deps", package_spec, "--progress-bar", "on"],
    ...
    req_path = os.path.join(sys.prefix, "requirements_v1.8.7.txt")
    if not os.path.exists(req_path):
        log(f"ERROR: requirements_v1.8.7.txt not found at {req_path}")
    ...
    # === Phase 6: Post-Install Verification ===
    log_section("Phase 6: Post-Install Verification")
    log("Verifying installation integrity...")
    verification_results = run_post_install_verification()
    passes, fails, warnings = verification_results

    log("")
    log("  " + "=" * 40)
    if fails == 0:
        log(f"  Verification: {passes} passed, {len(warnings)} warnings")
    else:
        log(f"  Verification: {passes} passed, {fails} FAILED, {len(warnings)} warnings")
        log("  Some components may not work correctly.")
    ...
    print_installation_summary(install_start_time, verification_results)

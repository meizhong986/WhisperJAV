...
1. QWEN3-ASR PIPELINE (NEW)
   - New Qwen3-based ASR engine with three input modes:
     Assembly (decoupled text + alignment), Context-Aware, and VAD-Slicing
   - ForcedAligner for precise word-level timestamps
   - Generation safety: repetition penalty + dynamic token budgets

2. ADAPTIVE STEP-DOWN ARCHITECTURE (NEW)
   - Tier 1/Tier 2 grouping with automatic fallback
   - Tighter scene/group boundaries for improved accuracy
   - Proportional timing fallback for alignment failures

3. PIPELINE ANALYTICS & HEALTH CHECK (NEW)
   - Real-time health metrics during pipeline execution
   - Per-group diagnostic detail records
   - Structured metadata for benchmarking and comparison

4. ALIGNMENT SENTINEL (NEW)
   - Automatic detection of collapsed/degenerate word timestamps
   - VAD-guided redistribution (Strategy C) for recovery
   - Cluster collapse detection via degenerate-ratio check

5. ENSEMBLE PIPELINE & GUI
   - Multi-pass processing with configurable pipeline combinations
...
3. Optional: Local LLM Installation (Requires User Acceptance)
   - You will be prompted to optionally install a local Large Language Model
   - This enables offline AI-powered features without requiring API keys
   - Requires additional ~4-8 GB disk space
   - You can skip this step and install later if needed
   - NOTE: This step requires user interaction (Yes/No prompt)
...
CUDA GPU ACCELERATION:
  ...
  - Driver 450+: PyTorch with CUDA 11.8 support (broad compatibility)
  - Driver 570+: PyTorch with CUDA 12.8 support (RTX 50-series, optimal)
...
Key Changes in 1.8.7b2:
  ...
  - Alignment sentinel for collapse detection and recovery
  - Ensemble pipeline with multi-pass processing

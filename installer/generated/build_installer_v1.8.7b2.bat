...
REM It MUST be bundled with the installer - no pip fallback allowed
REM Strategy: 1) Local copy, 2) Common install locations, 3) PATH, 4) Download, 5) FAIL

set "UV_FOUND=0"

REM === Step 1: Check if uv.exe already exists in current directory ===
if exist "uv.exe" (
    ...
    if not errorlevel 1 (
        ...
        echo   - Local uv.exe exists but is not functional, removing...
        del uv.exe 2>nul
    )
)

REM === Step 2: Check common installation locations (not just PATH) ===
echo   - Checking common installation locations...
set "UV_LOCATIONS="
set "UV_LOCATIONS=!UV_LOCATIONS! %USERPROFILE%\.local\bin\uv.exe"
set "UV_LOCATIONS=!UV_LOCATIONS! %USERPROFILE%\.cargo\bin\uv.exe"
set "UV_LOCATIONS=!UV_LOCATIONS! %LOCALAPPDATA%\uv\uv.exe"
set "UV_LOCATIONS=!UV_LOCATIONS! %APPDATA%\uv\uv.exe"
set "UV_LOCATIONS=!UV_LOCATIONS! %LOCALAPPDATA%\Programs\uv\uv.exe"

for %%p in (!UV_LOCATIONS!) do (
    if exist "%%p" (
        echo   - Found uv at: %%p
        echo   - uv: found at %%p >> "%BUILD_LOG%"
        copy "%%p" "uv.exe" >nul
        if exist "uv.exe" (
            uv.exe --version >nul 2>nul
            if not errorlevel 1 (
                ...
                echo   - uv: !UV_VER! (copied from %%p) >> "%BUILD_LOG%"
                set "UV_FOUND=1"
                goto :UvVerify
            )
        )
...
REM === Step 3: Check system PATH with 'where uv' ===
echo   - Checking system PATH...
where uv >nul 2>nul
if not errorlevel 1 (
    for /f "tokens=*" %%a in ('where uv') do (
        set "SYSTEM_UV=%%a"
        echo   - Found in PATH: !SYSTEM_UV!
        echo   - uv: found in PATH at !SYSTEM_UV! >> "%BUILD_LOG%"
        copy "!SYSTEM_UV!" "uv.exe" >nul
        if exist "uv.exe" (
            uv.exe --version >nul 2>nul
            if not errorlevel 1 (
                for /f "tokens=*" %%b in ('uv.exe --version 2^>^&1') do set UV_VER=%%b
                echo   - Copied to local: !UV_VER!
                echo   - uv: !UV_VER! (copied from PATH) >> "%BUILD_LOG%"
                set "UV_FOUND=1"
                goto :UvVerify
            )
        )
...
REM === Step 4: Download from GitHub ===
:UvTryDownload
echo   - uv not found locally, downloading from GitHub...
...
REM Note: The zip structure may vary between versions, so we search for uv.exe
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "    if ($uvExe) { " ^
    "        Copy-Item -Path $uvExe.FullName -Destination 'uv.exe' -Force; " ^
    "        Write-Host ('    Found and copied: ' + $uvExe.FullName) " ^
    "    } else { " ^
    "        Write-Host '    ERROR: uv.exe not found in downloaded archive'; " ^
    "        exit 1 " ^
    "    }; " ^
...
if exist "uv.exe" (
    ...
    if not errorlevel 1 (
        ...
        echo   - Downloaded uv.exe but it's not functional
        echo   - uv: download failed (not functional) >> "%BUILD_LOG%"
        del uv.exe 2>nul
    )
)

REM === Step 5: FAIL - uv.exe could not be obtained ===
echo.
echo ===============================================================================
echo   FATAL ERROR: Failed to obtain uv.exe!
echo ===============================================================================
echo.
echo   uv.exe is REQUIRED for building the installer. Without uv, users would
echo   experience 10-30x slower installation times with pip.
echo.
echo   Attempted locations:
echo     - Current directory: NOT FOUND
echo     - %%USERPROFILE%%\.local\bin\uv.exe: NOT FOUND
echo     - %%USERPROFILE%%\.cargo\bin\uv.exe: NOT FOUND
echo     - %%LOCALAPPDATA%%\uv\uv.exe: NOT FOUND
echo     - System PATH (where uv): NOT FOUND
echo     - GitHub download: FAILED
echo.
echo   Solutions:
echo     1. Install uv: powershell -c "irm https://astral.sh/uv/install.ps1 | iex"
echo     2. Check internet connection and firewall settings
echo     3. Manually download uv.exe and place it in: %CD%
echo        Download from: https://github.com/astral-sh/uv/releases
echo.
echo ERROR: uv.exe could not be obtained >> "%BUILD_LOG%"
pause
exit /b 1

REM === Final verification that uv.exe exists and works ===
:UvVerify
if not exist "uv.exe" (
    echo.
    echo FATAL ERROR: uv.exe does not exist after obtain phase!
    echo ERROR: uv.exe missing after obtain phase >> "%BUILD_LOG%"
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo.
    echo FATAL ERROR: uv.exe exists but is not functional!
    echo ERROR: uv.exe not functional >> "%BUILD_LOG%"
    del uv.exe 2>nul
    pause
    exit /b 1
)

echo   - uv.exe verified and ready
echo.

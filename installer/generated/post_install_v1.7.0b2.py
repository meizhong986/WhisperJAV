"""
WhisperJAV v1.7.0b2 Post-Install Script
======================================
...
4. Installs all Python dependencies from requirements_v1.7.0b2.txt
5. Installs WhisperJAV from GitHub
6. Creates desktop shortcut
7. Provides detailed installation summary

All output is logged to install_log_v1.7.0b2.txt
"""
...
LOG_FILE = os.path.join(sys.prefix, "install_log_v1.7.0b2.txt")
...
def create_failure_file(error_message: str):
    ...
    failure_file = os.path.join(sys.prefix, "INSTALLATION_FAILED_v1.7.0b2.txt")
    try:
        with open(failure_file, "w", encoding="utf-8") as f:
            f.write("WhisperJAV v1.7.0b2 Installation Failed\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Error: {error_message}\n\n")
            f.write("Manual Cleanup Steps:\n")
            f.write(f"1. Delete installation directory: {sys.prefix}\n")
            f.write("2. Delete desktop shortcut: WhisperJAV v1.7.0b2.lnk\n")
            f.write(f"3. Check install_log_v1.7.0b2.txt for details\n\n")
...
def print_installation_summary(install_start_time: float):
    ...
    log("\n\n")
    log("=" * 80)
    log(" " * 20 + "WhisperJAV v1.7.0b2 Installation Complete!")
    log("=" * 80)
    log("")
...
def main() -> int:
    ...
    log_section("WhisperJAV v1.7.0b2 Post-Install Started")
    ...
    req_path = os.path.join(sys.prefix, "requirements_v1.7.0b2.txt")
    if not os.path.exists(req_path):
        log(f"ERROR: requirements_v1.7.0b2.txt not found at {req_path}")

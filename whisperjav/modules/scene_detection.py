...
class SceneDetector:
    ...
    def detect_scenes(self, audio_path: Path, output_dir: Path, media_basename: str) -> List[Tuple[Path, float, float, float]]:
        ...
        logger.info(f"Starting two-pass audio scene detection for: {audio_path}")
        ...
        logger.info("Pass 1: Coarse splitting")
        ...
        logger.info(f"Pass 1 found {len(regions)} regions")
        ...
        for region_idx, region in enumerate(tqdm(regions, desc="Processing regions")):
            ...
            if region_duration <= self.max_duration:
                ...
                logger.info(f"Pass 2: Splitting long region {region_idx} ({region_duration:.1f}s)")
                ...
                if not sub_regions:
                    ...
                    logger.info(f"Pass 2 split region into {len(sub_regions)} sub-scenes")

...
class TimingAdjuster:
    ...
    def adjust_timings_content_aware(self,
        ...
        logger.info(f"STRICT timing adjustment complete:")
        logger.info(f"  Condition (a) - Substantial content changed: {condition_a_count} subtitles")
        logger.info(f"  Condition (b) - Duration > {self.HALLUCINATION_THRESHOLD}s: {condition_b_count} subtitles")
        logger.info(f"  Condition (c) - Merged lines detected: {condition_c_count} subtitles")
        logger.info(f"  Condition (d) - Abnormally Slow CPS (< {min_cps} CPS): {condition_d_count} subtitles")
        logger.info(f"  Untouched (as required): {untouched_count} subtitles")

...
class HallucinationRemover:
    ...
    def _load_patterns_safe(self):
        ...
        try:
            ...
            logger.info("Using built-in fallback patterns")
    ...
    def _load_fallback_patterns(self):
        ...
        logger.info(f"Loaded fallback patterns: {len(self._exact_lists)} exact lists, {len(self._regex_patterns)} regex patterns")
    ...
    def _load_patterns(self):
        ...
        try:
            ...
            if self._exact_lists:
                ...
                logger.info(f"Loaded exact hallucination lists for {len(self._exact_lists)} languages ({total_phrases} total phrases)")
            else:
        ...
        try:
            ...
            if pattern_data and 'patterns' in pattern_data:
                ...
                logger.info(f"Loaded {len(self._regex_patterns)} regex patterns")
                
                # Extract phrases for fuzzy matching
                self._blacklist_phrases = self._extract_blacklist_phrases(self._regex_patterns)
                logger.info(f"Extracted {len(self._blacklist_phrases)} phrases for fuzzy matching")
            else:

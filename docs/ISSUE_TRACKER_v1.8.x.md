...
> Updated: 2026-05-07 (rev49.5) | Source: [GitHub Issues](https://github.com/meizhong986/WhisperJAV/issues) | **69 open** on GitHub
...
### NEW P0 — v1.8.13 ensemble fidelity→balanced catastrophe (non-deterministic, multi-user)

| Priority | Item | Issues | Notes |
|---|---|---|---|
| **P0 NEW (2026-05-07)** | v1.8.13 ensemble pass1=fidelity + pass2=balanced intermittently produces catastrophically truncated pass 2 SRT | (no GitHub issue yet — owner reports + multiple user reports) | **Bug confirmed in v1.8.13 production.** GUI defaults configuration. Same config produces correct OR catastrophic output across runs (non-deterministic). Catastrophic case: pass 2 returns 14 entries vs expected ~50 on the 293s reference clip. ASR-layer divergence: whisperseg correctly finds VAD segments on heavier scenes (8-13), but faster-whisper / ctranslate2 returns 0 raw segments per scene, taking 33-67s per scene. Aggressive `no_speech_threshold=0.84` then gates the empty result. Investigation memo: `docs/plans/V1814_T142_NONDETERMINISM_INVESTIGATION.md`. Test suite for failure-rate measurement: `tools/ensemble_failure_rate_suite.py`. **Open hypothesis (not verified)**: probabilistic numerical drift in ctranslate2 inference, fidelity (heaviest pass 1) increases trigger probability via cuDNN auto-tune nondeterminism / GPU thermal / allocator state. Tests T142-T144 ruled out: deterministic state contamination, cross-engine alone, pass 2 alone, whisperseg state, config routing. Pending: failure-rate measurement (test suite running), `cudnn.deterministic=True` experiment, code archaeology on subprocess CUDA cleanup. |

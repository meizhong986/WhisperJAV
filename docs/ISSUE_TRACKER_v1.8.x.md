...
> Updated: 2026-05-10 (rev49.8) | Source: [GitHub Issues](https://github.com/meizhong986/WhisperJAV/issues) | **78 open** on GitHub
...
|----------|------:|-------|
| Total open on GitHub | **78** | **+8 net since rev49.6** (70→78). **+8 new** post-rev49.6 issues since 2026-05-05: **#321** Kaggle qwen Pass 1 FAIL — HF Hub unauthenticated warning (jasial2), **#322** async-processing usage Q on 3070/8GB (kokayin), **#323** WhisperSeg/TEN VAD fall back to silero in `--mode fidelity` outside `--ensemble` — hits the v1.8.13 scoped firewall (TinyRick1489), **#324 [Bug, candidate P0]** Korean transcription regression v1.8.10.post3 → v1.8.13: nearly all output flagged `[REMOVED - Symbol Only Residue]` (micky0526), **#325** DeepSeek model-name update — `deepseek-v4-flash`/`deepseek-v4-pro` replace `deepseek-chat`/`deepseek-reasoner` deprecating 2026-07-24 (1Dreamer666), **#326** "1x00 series GPU CUDA errors" — likely CC 6.1 cluster of #304/#286 (techguru0, sparse log), **#327** Suggestion: add CrispASR (parakeet+WhisperSeg) — v1.9.x backend FR (rikimtasu), **#328** FR: skip-or-overwrite policy for batch when matching .srt exists (unretired1516). **0 closed since rev49.6.** Open issues with new comments needing maintainer follow-up: **#309** Jerry199022 reported run.bat workaround works for GUI launch but Save Preset writes to mklink-junctioned `C:\Users\Jerry\AppData\…\presets\1.json` (NEW bug surfaced 2026-05-10), **#316** bruce609685-collab clarified the request is **user-marked plot/act boundaries that override the auto time-based segmenter** — not source-video cutting (different ask than the 2026-05-05 reply addressed), **#312** TinyRick1489 reports `--dump-params` returns near-empty payload (resolved_config null, all cli_args null) in `--ensemble` mode — NEW bug post v1.8.13 ship, **#308** SangenBR replied 2026-05-06 with thanks acknowledgement only. |
...
| **NEEDS RESPONSE** | **11** | **8 new since rev49.6**: **#321** (Kaggle qwen HF_TOKEN), **#322** (async-processing Q), **#323** (whisperseg/ten outside ensemble — point to v1.9.0 unified-routing roadmap), **#324** (P0 candidate — Korean sanitizer regression v1.8.13), **#325** (DeepSeek model-name update — small providers.py edit), **#326** (1x00 GPU CUDA, sparse log — info-gather), **#327** (CrispASR FR — v1.9.x backlog), **#328** (.srt skip/overwrite FR). **3 carried from rev49.6**: **#314** (install Phase 3 hang), **#317** (installer proxy Q), **#320** (macOS SSL CERT_VERIFY_FAILED). |
| **NEEDS FOLLOW-UP** | **5** | **#309** (Jerry199022 — run.bat workaround works, but Save Preset fails to mklink-junctioned presets dir; new sub-bug 2026-05-10), **#312** (TinyRick1489 — `--dump-params` ensemble-mode null payload, new bug 2026-05-06), **#316** (bruce609685-collab — clarified ask is user-marked plot boundaries, not source split; corrected understanding needed), **#308** (SangenBR ack only — light-touch close-or-ack), **#231** (fzfile Kaggle notebook clarification, carried from rev49.6). |
...
## v1.8.14 — IN-FLIGHT (rev49.7 theme refresh, theme: ASR preset retune + GUI update-check UX + ensemble catastrophe cap; Cohere DEFERRED to v1.9.0)

> **Theme refresh 2026-05-10**: Cohere is **DEFERRED to v1.9.0**. Rev49.4's "ChronosJAV" theme
> shipped through code (8 commits, all on `dev_v1.8.14`) but is **dormant** — GUI dropdown options
> are `disabled` and a defensive `transformers>=4.40.0,<5.0` pin in `pyproject.toml` +
> `installer/core/registry.py` blocks the upgrade path that would activate Cohere. Rationale:
> Cohere requires transformers ≥5.4.0, but transformers 5.x breaks the bundled Qwen3-ASR fork
> (`@check_model_inputs()` decorator API change at `modeling_qwen3_asr.py:986`). Empirical
> evidence in `test_media/1814_acceptance/F2/log_testrun.txt`. Upstream issue posted
> 2026-05-08 on QwenLM/Qwen3-ASR#138 awaiting maintainer response. Full v1.9.0 carryover
> in `memory/project_v1814_cohere_deferred_v19x_carryover.md`.
>
> **Effective v1.8.14 headline themes (load-bearing for ship)**:
> 1. **conditional_sensitivity_cap** for v1.8.13 ensemble fid→bal+aggressive catastrophe (commit `5e8da48`, shipped earlier in v1.8.14 dev — defense-in-depth insurance, kept dormant after retune).
> 2. **Comprehensive FW + Whisper ASR preset retune** across all 3 sensitivities (commit `d0bb379`, 2026-05-09). Engine-split discipline preserved. R1-R5 manual tests passed per owner 2026-05-10. See `memory/project_v1814_aggressive_retune_in_flight.md` (now historical).
> 3. **GUI Check-for-Updates redirect** — retire in-GUI pip-install upgrade path; users sent to GitHub releases page (commit `ea01a07`, 2026-05-10). Addresses fragility / bad-UX user reports on the in-GUI updater.
> 4. **Install UX polish carry-overs** — #314, #317, #320, #280 (still pending).

### v1.8.13 ensemble fidelity→balanced catastrophe — FIXED in v1.8.14 dev (commit 5e8da48)

| Priority | Item | Issues | Notes |
|---|---|---|---|
| **P0 FIXED (2026-05-07)** | v1.8.13 ensemble pass1=fidelity + pass2=balanced intermittently produces catastrophically truncated pass 2 SRT | (no GitHub issue yet — owner reports + multiple user reports) | **FIX SHIPPED in v1.8.14 dev** (commit `5e8da48` on `dev_v1.8.14`). Pattern: **conditional_sensitivity_cap** in `whisperjav/ensemble/safety_caps.py`. When the trigger combination is detected (pass1=fidelity + pass2=balanced + sensitivity=aggressive), pass 2 sensitivity is auto-downgraded to `balanced` (single temperature, lower no_speech_threshold) — eliminating the catastrophic-empty-scene manifestation. **Empirical validation**: 22/22 runs catastrophic-free on owner's hardware (10 A_fid_bal + 10 E_fid_bal_BAL + 2 F_fid_bal_CON; F suite cancelled at 2 because signal was already sufficient). Pass 2 wall-clock now deterministic at 62-68s; pass 2 entry count consistent at 47 every run. Investigation memo: `docs/plans/V1814_T142_NONDETERMINISM_INVESTIGATION.md` (sections 1-20). FAQ entry added to `docs/en/faq.md` covering both the cap behavior and the orthogonal pass 1 fidelity timing variance (4.6x range due to PyTorch cuDNN auto-tune — documented as upstream characteristic, optimization deferred to v1.9.0+). **Architectural root cause** (CTranslate2 / cuBLAS internal nondeterminism + temperature=0.17 sampling fallback) deferred to v1.9.0+ research. **Reorder-as-alternative-fix** considered and dropped for v1.8.14 (untested, more invasive); filed as v1.9.0+ research candidate (memo §20). |
...
| **#321** | Got error while using qwen in kaggle | jasial2 | `NEEDS RESPONSE` | **NEW 2026-05-05.** Pass 1 FAIL with: "Error: Warning: You are sending unauthenticated requests to the HF Hub. Please set a HF_TOKEN to enable higher rate limits and faster downloads." Suggests Kaggle qwen pipeline doesn't surface HF_TOKEN requirement clearly (may treat the warning as fatal). Reply: HF_TOKEN setup steps for Kaggle (Add-ons → Secrets → HF_TOKEN), reload kernel; document in Kaggle notebook header. **Cluster F (Kaggle)**. **v1.8.14 candidate** — small UX/doc fix; could also surface "set HF_TOKEN" as actionable error message. |

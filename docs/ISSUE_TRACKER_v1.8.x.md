...
| **NEEDS RESPONSE** | **2** | **#304, #311.** **rev46 batch flipped 5 → 2**: #305 (CN→EN translation), #309 (mklink white screen), #312 (VAD dump_params), #307 (sanitization Q) all responded; #313 + #300 responded earlier in rev46. Remaining: #304 (GTX 1050 Ti compat), #311 (FireRedVAD feature request). |
...
| **#300** | v1.8.11 install.py issue | ktrankc | `AWAITING REPLY` | **NEW 04-21.** uv resolver deadlock: `whisperjav[all]` → `bs-roformer-infer` → `requests>=2.31`, but pytorch index pins `requests==2.28.1`. User reverted to 1.8.10.post3. **Responded 04-30** ([comment-4358567338](https://github.com/meizhong986/WhisperJAV/issues/300#issuecomment-4358567338)): `UV_INDEX_STRATEGY=unsafe-best-match python install.py` workaround + v1.8.13 fix promise. Cluster with #313. |

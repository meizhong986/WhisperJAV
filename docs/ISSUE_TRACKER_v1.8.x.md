...
> Updated: 2026-05-11 (rev49.10) | Source: [GitHub Issues](https://github.com/meizhong986/WhisperJAV/issues) | **79 open** on GitHub
...
| `NEEDS DIAGNOSTIC DATA` | Symptom reported across versions/configs; cause unverified; awaiting log/test data from reporter. |
...
|----------|------:|-------|
| Total open on GitHub | **79** | **+1 net since rev49.8** (78 → 79). **+2 new** post-rev49.8 issues, both 2026-05-11: **#329** fzfile Kaggle Pass 1 timeout at 7200s, 0 replies; **#330** jasial2 (same reporter as the SHIPPED #321) Kaggle parallel mode all-passes-FAIL with "no log captured", 0 replies. **0 closures** since rev49.8 (most recent close was #304 on 2026-05-05). rev49.8 listed 78 open; rev49.8 + 2 new = expected 80, live is 79 — rev49.8 count was likely off-by-one. |
| **v1.8.14 RELEASED** | — | **2026-05-11** (published 2026-05-10T17:27Z UTC; owner local 2026-05-11). Tag `v1.8.14` annotated on `main` commit `f7862f7`. Release URL <https://github.com/meizhong986/WhisperJAV/releases/tag/v1.8.14>. Pre-release `false`, draft `false`. **Assets**: `WhisperJAV-1.8.14-Windows-x86_64.exe` (uploaded 05-10T17:25Z, 309,717,245 bytes) + `whisperjav-1.8.14-py3-none-any.whl` (uploaded 05-11T12:19Z, 1,331,152 bytes). 27 commits ahead of v1.8.13. **Theme**: quality hardening — `conditional_sensitivity_cap` (commit `5e8da48`) + comprehensive ASR preset retune (`d0bb379`) + Korean sanitizer fix #324 (`3ea442d`) + DeepSeek default bump #325 (`f234610`) + Kaggle diagnostics #321 (`5a0b223`) + `--dump-params` ensemble fix #312 (`361ad1c`) + GUI Check-for-Updates redirect (`ea01a07`) + install-troubleshooting FAQ #280/#314/#317/#320 (`e1cddf0`). **Cohere DEFERRED to v1.9.0** — 5 commits dormant on `main` behind `disabled` GUI dropdown (`webview_gui/assets/index.html:401,557`) + defensive `transformers<5.0` pin. See `memory/project_v1814_cohere_deferred_v19x_carryover.md`. |
...
| **NEEDS RESPONSE** | **7** | **rev49.9 recount** (rev49.8 had 11): **6 flipped to SHIPPED v1.8.14** (#324, #325, #321, #314, #317, #320); **+2 new** post-rev49.8 (#329 fzfile Kaggle 2h timeout 2026-05-11, #330 jasial2 Kaggle "no log captured" 2026-05-11). **Remaining 7**: #322 (async-processing Q), #323 (intentional firewall — point to v1.9.0), #326 (1x00 GPU CUDA, sparse log), #327 (CrispASR FR — v1.9.x), #328 (.srt skip/overwrite FR), #329, #330. |
| **NEEDS FOLLOW-UP** | **4** | **rev49.9 recount** (rev49.8 had 5): #312 flipped to SHIPPED v1.8.14. **Remaining 4**: #309 (mklink presets sub-bug 2026-05-10), #316 (corrected plot-boundary understanding needed), #308 (ack-only close-or-ack), #231 (Kaggle notebook clarification carry-over). |
| **NEEDS DIAGNOSTIC DATA** | **1** | **#297** teijiIshida — blank/truncated SRT observed across v1.8.11 / v1.8.12 / v1.8.14 and across 3 segmenters (defaults / silero-v6.2 / whisperseg). Two most recent runs ended at 44m, but v1.8.11 test ended at 58m → 44m is NOT a universal fixed cap. Reporter confirmed fresh `.exe` install of v1.8.14 ([comment-4420874603](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4420874603)). Owner asked for debug log + balanced-sensitivity A/B ([comment-4418826539](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4418826539), [comment-4421205501](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4421205501)); reporter committed to retest. **NOT in v1.8.14 fix list** — do not flip to SHIPPED. **Hypothesis (NOT verified)**: none offered — insufficient data. |
| **SHIPPED v1.8.14 — notified, awaiting retest** | **8** | #324, #325, #321, #312, #314, #317, #320, #280 — all received FIX CODED replies 2026-05-10 + v1.8.14 ship-notification replies 2026-05-11 (URLs in rev49.10 changelog entry). Awaiting reporter retests; no closures pending until reporters confirm fix on their end. |
| **AWAITING REPLY** | ~27 | Carry-over from rev49.8. **#297 moved OUT** to NEEDS DIAGNOSTIC DATA. |
...
## v1.8.14 — RELEASED (2026-05-11)

Tagged `v1.8.14` on `main` commit `f7862f7`, 2026-05-11 (owner local; GitHub publish 2026-05-10T17:27Z UTC). GitHub Release URL: <https://github.com/meizhong986/WhisperJAV/releases/tag/v1.8.14>. Pre-release `false`, draft `false`. Assets attached: `WhisperJAV-1.8.14-Windows-x86_64.exe` (uploaded 05-10T17:25Z, 309,717,245 bytes) + `whisperjav-1.8.14-py3-none-any.whl` (uploaded 05-11T12:19Z, 1,331,152 bytes — ~19h post-publish; earlier rev49.8-era memory entries stating "wheel was NOT uploaded" are stale and were superseded before reporter retest activity began).

**Theme**: quality hardening on top of v1.8.13. conditional_sensitivity_cap + comprehensive ASR preset retune + Korean sanitizer fix (#324) + DeepSeek default bump (#325) + Kaggle notebook diagnostic improvements (#321) + `dump_params` ensemble fix (#312) + GUI Check-for-Updates redirect + install-troubleshooting FAQ entries (#280, #314, #317, #320).

**Cohere generator DEFERRED to v1.9.0**: 5 dormant commits on `main`; GUI dropdown options carry HTML `disabled` attribute (`webview_gui/assets/index.html:401`, `index.html:557`); defensive `transformers>=4.40.0,<5.0` pin in `pyproject.toml` and `installer/core/registry.py`. Rationale: Cohere requires transformers ≥5.4.0, which breaks the bundled Qwen3-ASR fork's `@check_model_inputs()` decorator (`modeling_qwen3_asr.py:986`). Upstream issue QwenLM/Qwen3-ASR#138 (owner comment 2026-05-08) awaiting maintainer. Full v1.9.0 carryover: `memory/project_v1814_cohere_deferred_v19x_carryover.md`.

> **Effective v1.8.14 ship-bearing items**:
> 1. **conditional_sensitivity_cap** for v1.8.13 ensemble fid→bal+aggressive catastrophe (commit `5e8da48`). Defense-in-depth insurance; kept dormant after retune.
> 2. **Comprehensive FW + Whisper ASR preset retune** across all 3 sensitivities (commit `d0bb379`, 2026-05-09). Engine-split discipline preserved. Owner R1-R5 manual tests passed 2026-05-10.
> 3. **GUI Check-for-Updates redirect** (commit `ea01a07`, 2026-05-10) — in-GUI pip-install path retired; users sent to GitHub releases page.
> 4. **Korean sanitizer regression fix** (#324, commit `3ea442d`) — `_HAS_LINGUISTIC_CONTENT` regex extended with Hangul + 9 other non-CJK scripts. 11 regression tests, full suite 48/48 PASS.
> 5. **Install UX FAQ entries** (#280, #314, #317, #320, commit `e1cddf0`); DeepSeek default bump (#325, commit `f234610`); Kaggle Pass 1/2 diagnostics + HF_TOKEN doc (#321, commit `5a0b223`); `--dump-params` ensemble fix (#312, commit `361ad1c`).

### v1.8.13 ensemble fidelity→balanced catastrophe — FIXED in v1.8.14 dev (commit 5e8da48)

| Priority | Item | Issues | Notes |
|---|---|---|---|
| **P0 SHIPPED v1.8.14** | v1.8.13 ensemble pass1=fidelity + pass2=balanced intermittently produces catastrophically truncated pass 2 SRT | (no GitHub issue yet — owner reports + multiple user reports) | **FIX SHIPPED in v1.8.14** (commit `5e8da48`). Pattern: **conditional_sensitivity_cap** in `whisperjav/ensemble/safety_caps.py`. When the trigger combination is detected (pass1=fidelity + pass2=balanced + sensitivity=aggressive), pass 2 sensitivity is auto-downgraded to `balanced` (single temperature, lower no_speech_threshold). **Empirical validation**: catastrophe-free in 22/22 runs on owner's hardware (10 A_fid_bal + 10 E_fid_bal_BAL + 2 F_fid_bal_CON; F suite cancelled at 2 because signal was already sufficient). Pass 2 wall-clock deterministic at 62-68s in those runs; pass 2 entry count consistent at 47. **Cross-machine validation pending.** Investigation memo: `docs/plans/V1814_T142_NONDETERMINISM_INVESTIGATION.md` (sections 1-20). FAQ entry added to `docs/en/faq.md` covering both the cap behavior and the orthogonal pass 1 fidelity timing variance (4.6x range due to PyTorch cuDNN auto-tune — documented as upstream characteristic, optimization deferred to v1.9.0+). **Architectural root cause** (CTranslate2 / cuBLAS internal nondeterminism + temperature=0.17 sampling fallback) deferred to v1.9.0+ research. **Reorder-as-alternative-fix** considered and dropped for v1.8.14 (untested, more invasive); filed as v1.9.0+ research candidate (memo §20). |

### Cohere generator — DEFERRED to v1.9.0

5 commits scaffolding Cohere are on `main` but dormant:
- `88ebeb6` feat(chronosjav): scaffold Cohere TextGenerator (Day 1/4)
- `421af9c` feat(chronosjav): Cohere generate() body + tests + smoke test (Day 2/4)
- `565deea` feat(chronosjav): Cohere GUI plumbing + FAQ EN/ZH (Day 3/4)
- `1db2cc4` fix(chronosjav/cohere): four robustness fixes after first live load
- `f395983` defer(chronosjav): grey out Cohere preview + pin transformers<5.0

GUI dropdown options for `cohere` carry the HTML `disabled` attribute at `webview_gui/assets/index.html:401` and `:557`. `pyproject.toml` and `installer/core/registry.py` pin `transformers>=4.40.0,<5.0`.

**Hard prerequisites for v1.9.0 ship**: (1) Qwen3-ASR fork patch for transformers 5.x `@check_model_inputs` decorator API; (2) verify Qwen3-ASR output accuracy on 5.x (upstream community report cites degradation after the decorator fix); (3) coordinated transformers ≥5.4.0 upgrade in `pyproject.toml` + `installer/core/registry.py`; (4) regression-test all 5 ASR backends on transformers 5.x; (5) un-disable GUI dropdown options; (6) drop forced FP16 in `cohere.py.load()` (let transformers 5.x auto-load bf16 from config.json).

Full archive: `memory/project_v1814_cohere_deferred_v19x_carryover.md`. Upstream tracking: QwenLM/Qwen3-ASR#138 (owner comment 2026-05-08 awaiting maintainer).

### Install UX polish — SHIPPED v1.8.14

| Priority | Item | Issues | Status | Notes |
|---|---|---|---|---|
| **P1** | Installer Phase 3 PyTorch silence | #314 | `SHIPPED v1.8.14 (notified, awaiting retest)` | doc-only fix in commit `e1cddf0`: FAQ entry expanded with Phase 3 progress-checking + tail-the-log guidance. Code-side streaming fix DEFERRED to v1.9.0 install hardening track. FIX CODED reply 2026-05-10 ([comment-4415377477](https://github.com/meizhong986/WhisperJAV/issues/314#issuecomment-4415377477)). |
| **P1** | Installer proxy support docs | #317 | `SHIPPED v1.8.14 (notified, awaiting retest)` | doc-only in commit `e1cddf0`: FAQ documenting `HTTP_PROXY`/`HTTPS_PROXY` env-var setup pattern. FIX CODED reply 2026-05-10 ([comment-4415377830](https://github.com/meizhong986/WhisperJAV/issues/317#issuecomment-4415377830)). |
| **P1** | macOS SSL CERTIFICATE_VERIFY_FAILED | #320 | `SHIPPED v1.8.14 (notified, awaiting retest)` | doc-only in commit `e1cddf0`: FAQ entry pointing to `/Applications/Python\ 3.12/Install\ Certificates.command` + note avoiding alpha Python builds. FIX CODED reply 2026-05-10 ([comment-4415380191](https://github.com/meizhong986/WhisperJAV/issues/320#issuecomment-4415380191)). |
| **P2** | Qwen3-ASR `transformers` version pin | #280 | `SHIPPED v1.8.14 (notified, awaiting retest)` | defensive `transformers>=4.40.0,<5.0` pin in `pyproject.toml` + `installer/core/registry.py` + FAQ entry in commit `e1cddf0`. FIX CODED reply 2026-05-10 ([comment-4415380555](https://github.com/meizhong986/WhisperJAV/issues/280#issuecomment-4415380555)). |
...
- Cohere generator (deferred to v1.9.0 — see "Cohere generator — DEFERRED to v1.9.0" section above)
- All v1.9.0 architectural items (large-v3 retune, firewall elimination, silero retune review)

## v1.9.0 — major release roadmap (additions surfaced in v1.8.13 dev + v1.8.14 Cohere deferral)

| Priority | Item | Issues | Effort | Notes |
|---|---|---|---|---|
| **P0 NEW (v1.8.14 deferral)** | Cohere generator ship (lifts v1.8.14 deferral) | #262 | Medium | All 5 Cohere commits dormant on `main` since v1.8.14 (`88ebeb6`, `421af9c`, `565deea`, `1db2cc4`, `f395983`). Ship prerequisites: (1) Qwen3-ASR fork patch for transformers 5.x decorator API; (2) verify Qwen3-ASR output accuracy on 5.x; (3) coordinated transformers ≥5.4.0 upgrade; (4) 5-backend regression test; (5) un-disable GUI dropdown options at `webview_gui/assets/index.html:401,557`; (6) drop forced FP16 in `cohere.py.load()` (let transformers 5.x auto-load bf16). See `memory/project_v1814_cohere_deferred_v19x_carryover.md`. Tracking: QwenLM/Qwen3-ASR#138. |
...
## Post-v1.8.13 Outstanding Actions (rev49.7, 2026-05-10 refresh)

### Triage queue — rev49.7 NEEDS RESPONSE (11) + NEEDS FOLLOW-UP (5)

**11 NEEDS RESPONSE** (8 new since rev49.6 + 3 carried):

| # | Reporter | Date | One-line | Priority candidate |
|---|----------|------|----------|--------------------|
| **#324** | micky0526 | 2026-05-06 | Korean v1.8.13 sanitizer regression — `[REMOVED - Symbol Only Residue]` | **P0 candidate** for v1.8.14 — investigation gating |
| **#325** | 1Dreamer666 | 2026-05-06 | DeepSeek model-name update — `deepseek-v4-flash`/`deepseek-v4-pro` | **P1** v1.8.14 — small providers.py edit |
| **#321** | jasial2 | 2026-05-05 | Kaggle qwen Pass 1 FAIL on HF Hub unauthenticated warning | **P1** v1.8.14 — doc + actionable error |
| **#314** | zoqapopita93 | 2026-05-01 | Installer Phase 3 progress feedback | **P1** v1.8.14 — install UX (carry-over) |
| **#317** | zoqapopita93 | 2026-05-03 | Installer proxy support docs / flag | **P1** v1.8.14 — China-network (carry-over) |
| **#320** | Madaerpao | 2026-05-05 | macOS SSL cert verify failed | **P1** v1.8.14 — doc fix (carry-over) |
| **#323** | TinyRick1489 | 2026-05-06 | WhisperSeg/TEN VAD outside `--ensemble` falls to silero | **P2** — intentional, point to v1.9.0 unified-routing |
| **#322** | kokayin | 2026-05-06 | Async-processing safety question on 3070/8GB | **P2** — Q&A only |
| **#326** | techguru0 | 2026-05-07 | "1x00 series GPU CUDA errors" sparse log | **P2** — info-gather (likely #304 cluster) |
| **#327** | rikimtasu | 2026-05-09 | Suggestion: add CrispASR | **P3** — v1.9.x backend expansion |
| **#328** | unretired1516 | 2026-05-09 | FR: skip-or-overwrite for batch | **P2** — small ergonomic; v1.8.14 if low-effort |

**5 NEEDS FOLLOW-UP** (user replied AFTER maintainer's last comment):

| # | Reporter | Last user reply | What's new | Priority candidate |
|---|----------|-----------------|------------|--------------------|
| **#312** | TinyRick1489 | 2026-05-06 | NEW BUG — `--dump-params` ensemble mode produces near-empty payload (resolved_config null, all cli_args null) | **P1** v1.8.14 if small fix |
| **#309** | Jerry199022 | 2026-05-10 | run.bat workaround (WEBVIEW2_USER_DATA_FOLDER + --no-sandbox) launches GUI; but Save Preset writes to mklink-junctioned path and fails | **P2** v1.8.14/v1.9.0 — connects to #96 settings persistence |
| **#316** | bruce609685-collab | 2026-05-07 | OP clarified original ask was misread — wants user-marked plot/act boundaries that override the auto segmenter, not source-video cutting | **P3** v1.9+/v2.x — corrected reply needed |
| **#308** | SangenBR | 2026-05-06 | Thanks/ack only on the Part-1 ship + ASS/SSA-not-planned framing | Light — close-or-ack |
| **#231** | fzfile | 2026-04-22 (carry-over) | "Still error prompts" — needs notebook-version clarification | Carry-over — chase user for notebook version |

---

### Immediate replies — drafts already prepared (rev49.6 historical)
...
|---|----------|-----|---------------------------------------|
| ~~**#304**~~ | ~~wenkine-2026~~ | ~~"Please close"~~ | **CLOSED 2026-05-05** ([comment-4380373098](https://github.com/meizhong986/WhisperJAV/issues/304#issuecomment-4380373098)) — brief ack of CC 6.1 hardware incompat. |
| **#315** | jasial2 | Kaggle expert translate broken — real bug | **FIXED 2026-05-05** off-release on `main` (commit [4103606](https://github.com/meizhong986/WhisperJAV/commit/4103606)). Replied with link. |
| **#318** | jasial2 | Kaggle expert qwen ASR missing | **FIXED 2026-05-05** off-release on `main` (commit [4103606](https://github.com/meizhong986/WhisperJAV/commit/4103606)). Replied with link. |
...
| ~~**#316**~~ | ~~bruce609685-collab~~ | ~~User-defined scene split FR~~ | **REPLIED 2026-05-05** ([comment-4380374226](https://github.com/meizhong986/WhisperJAV/issues/316#issuecomment-4380374226)) — ffmpeg manual split workaround + #230 pointer + v1.9+/v2.x backlog. |
| ~~**#319**~~ | ~~kskbl-manbo~~ | ~~AMD GPU FR~~ | **REPLIED 2026-05-05** ([comment-4380374766](https://github.com/meizhong986/WhisperJAV/issues/319#issuecomment-4380374766)) — pointed to #114/#142/#239 cluster, ROCm vs DirectML scope note. |
...
- **#287** zoqapopita93, **#294** CaliburnKoko, **#302** Kukuindi — all v1.8.12 retest pings posted 04-30. v1.8.13 shipped 05-05; users may need to retest both v1.8.12 fixes AND v1.8.13 model revert. If no retest data after 7-10 more days, follow up.
- **#297** teijiIshida — **MOVED to NEEDS DIAGNOSTIC DATA** (rev49.9, 2026-05-11). Reporter retested on v1.8.12 + v1.8.14 and reports continued truncation at 44m across different segmenters/files. Owner asked for debug log + balanced-sensitivity A/B; reporter committed to retest. Full evidence pointers in the bugs table row above. **NOT in v1.8.14 fix list — do not close.**
...
### v1.8.14 — LOCKED SCOPE (rev49.4, 2026-05-05) — historical, superseded by "v1.8.14 — RELEASED" section above

> **Note (rev49.9, 2026-05-11)**: The rev49.4 "ChronosJAV: Cohere generator" theme did NOT ship as planned — Cohere was deferred mid-cycle (see "v1.8.14 — RELEASED" section above for what actually shipped). The text below is preserved as the contemporaneous planning record only.
...
|---|-------|----------|--------|-------|
| **#330** | Still having issue with qwen in kaggle | jasial2 | `NEEDS RESPONSE` | **NEW 2026-05-11**, 0 replies. Same reporter as #321 (SHIPPED v1.8.14). Reporter ran Kaggle PARALLEL mode, 2 GPUs, 4 files. Files 1/4 (sw-613-45m.mp3) and 2/4 (sw-613-2h15m.mp3) explicitly shown failing both Pass 1 AND Pass 2 with `Error (last 1500 chars): (no log captured)`; reporter's `[1/4]`/`[2/4]` framing implies the pattern continued for files 3/4 and 4/4 (not separately quoted). Each pass ~485-490s (suggests timeout, not immediate crash). Reporter set HF_TOKEN per FAQ AND directly in cell. Reporter asks: "if we select pass quality as qwen, what model should be selected?" — UX confusion between qwen-model dropdown and pass-quality dropdown. **Hypothesis (NOT verified)**: "(no log captured)" could mean either (a) reporter's notebook clone does not include the #321 diagnostic improvement (commit `5a0b223`), or (b) the failure occurs at a point the diagnostic does not cover (e.g., subprocess timeout before stdout/stderr is captured). Neither verified. **Suggested next action**: ask reporter for notebook URL/commit + answer the qwen-model selection Q. Internally verify whether the Kaggle parallel notebook on `main` actually prints the "last 1500 chars" message or shows "no log captured" under timeout. |
| **#329** | kaggle default setting pass1 failed, timeout | fzfile | `NEEDS RESPONSE` | **NEW 2026-05-11**, 0 replies. Body verbatim: `Pass 1 (7200s): FAIL` / `7369.2s 59 Error (last 1500 chars):` / `7369.2s 60 Timeout expired`. 7200s = 2-hour timeout. No other error detail in the report. Same reporter as **#231** (Kaggle notebook errors, carry-over open since 2026-04-22). **Hypothesis (NOT verified)**: none. **Suggested next action**: reply asking for notebook URL/commit, full log, config screenshot, GPU info, file size, HF_TOKEN state. Cross-reference #231 (same reporter — possibly same Kaggle issue resurfacing). |
| **#328** | [FR] Skip-or-overwrite policy for batch when matching .srt exists | unretired1516 | `NEEDS RESPONSE` | **NEW 2026-05-09.** Two-line ask: "Please consider adding an option to ignore files that have an .srt file matching their filename, or overwrite them." Small ergonomic ask for batch / folder processing. Maps cleanly to a 2-flag CLI/GUI knob (`--skip-existing` / `--overwrite-existing`). v1.8.14 candidate if low-effort, otherwise v1.9.x backlog. |
| **#327** | [Suggestion] Add CrispASR (parakeet + WhisperSeg combo) | rikimtasu | `NEEDS RESPONSE` | **NEW 2026-05-09.** Points to https://github.com/CrispStrobe/CrispASR; OP wants to try parakeet paired with WhisperSeg VAD. **v1.9.x backlog candidate** — joins the ASR backend expansion track (#311 FireRedVAD, #262 Cohere [DEFERRED to v1.9.0], #205 VibeVoice, #232 anime-whisper variants). Reply: thanks + v1.9.x backend-expansion track positioning. |
| **#326** | not working on 1x00 series nvidia gpu | techguru0 | `NEEDS RESPONSE` | **NEW 2026-05-07.** Sparse report — "I get nothing but cuda errors trying to run the program with many different options tried". No log, no driver/CUDA version, no specific GPU model. **Same reporter as the (closed) #286** GTX 1050 Ti CC 6.1 incompat. Likely the same cluster (Pascal CC 6.1 not in shipped torch wheel). Reply: ask for full console error + driver version + nvidia-smi output; point to #304/#286 hardware-incompat history. |
| **#325** | DeepSeek 更新模型名称了，需要修改 translate 中的设置 | 1Dreamer666 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-06.** **Real bug, small fix.** DeepSeek announced (https://api-docs.deepseek.com/zh-cn/) new model names: `deepseek-v4-flash` (non-thinking) and `deepseek-v4-pro` (thinking). The current `deepseek-chat` and `deepseek-reasoner` will be **deprecated 2026-07-24**. Action: update `whisperjav/translate/providers.py` DeepSeek model list. **SHIPPED v1.8.14** (commit `f234610` — providers.py default model bumped to `deepseek-v4-flash`; OpenRouter route left at legacy name pending upstream change). FIX CODED reply 2026-05-10 ([comment-4415376078](https://github.com/meizhong986/WhisperJAV/issues/325#issuecomment-4415376078)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
| **#324** | [Bug] Korean transcription regression v1.8.13: Native subtitles flagged as "[REMOVED - Symbol Only Residue]" | micky0526 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-06. P0 CANDIDATE — possible v1.8.13 regression.** Reporter compares **v1.8.10.post3 (works)** vs **v1.8.13 (breaks)** on **Korean** source audio with identical settings (Transcription mode, balanced model, aggressive sensitivity, native subs). On v1.8.13: nearly all output flagged `[REMOVED - Symbol Only Residue]` in `stitched.artifacts.srt`, leaving only 2-3 lines in final SRT. Reporter attached 5 files including v1.8.10.post3 working output and v1.8.13 broken output. **Hypothesis (UNVERIFIED)**: post-processing rule in subtitle_sanitizer.py mis-firing on Hangul. Possibly introduced by v1.8.11 sanitizer hardening (Fix 5a, commit 783b218 SDH patterns) or v1.8.13 polish bundle. **Investigation priority** before v1.8.14 ship: read attached artifacts, identify which sanitizer rule classifies Korean as "symbol only", patch the rule's Unicode range or relax the symbol-class detection. **SHIPPED v1.8.14** (commit `3ea442d` — `_HAS_LINGUISTIC_CONTENT` regex extended with Hangul + 9 other non-CJK scripts; 11 new regression tests; full suite 48/48 PASS). FIX CODED reply 2026-05-10 ([comment-4415375722](https://github.com/meizhong986/WhisperJAV/issues/324#issuecomment-4415375722)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
| **#323** | Can't use Whisperseg and TEN VAD without Ensemble mode | TinyRick1489 | `NEEDS RESPONSE` | **NEW 2026-05-06.** User reports `--mode fidelity --speech-segmenter ten` (or `whisperseg`) silently falls back to silero-v3.1 with WARNING. **This is intentional per the v1.8.13 scoped firewall** (`whisper_pro_asr.py:74-81` strips backend-agnostic grouping params for non-Silero backends in BalancedPipeline / FidelityPipeline; allow-list logic in `main.py:1840-1898`). The warning text already calls this out. **Reply**: confirm intentional, point to `--ensemble` workaround for full WhisperSeg/TEN/NeMo support, link to v1.9.0 unified-segmenter-routing roadmap (P0 in v1.9.0 — eliminates the firewall and lets WhisperSeg/TEN work everywhere). No code change for v1.8.14. |
| **#322** | When should I use async processing? | kokayin | `NEEDS RESPONSE` | **NEW 2026-05-06.** User Q on async-processing safety with RTX 3070 8GB, balanced/balanced 1-pass — "GPU ram usage is 6.2-6.4/8.0GB when running whisperjav, should I use async processing to make the stt process much faster? or it will lead to missing lines in ja.srt when vram is not enough?" Q&A only, no code change. Reply: explain async spawns parallel workers each holding ~6GB → 12GB+ doesn't fit 8GB → likely OOM crash or instability; recommend stick with sync at 6.2-6.4GB usage. Could become an FAQ entry. |
| **#321** | Got error while using qwen in kaggle | jasial2 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-05.** Pass 1 FAIL with: "Error: Warning: You are sending unauthenticated requests to the HF Hub. Please set a HF_TOKEN to enable higher rate limits and faster downloads." Suggests Kaggle qwen pipeline doesn't surface HF_TOKEN requirement clearly (may treat the warning as fatal). Reply: HF_TOKEN setup steps for Kaggle (Add-ons → Secrets → HF_TOKEN), reload kernel; document in Kaggle notebook header. **Cluster F (Kaggle)**. **SHIPPED v1.8.14** (commit `5a0b223` — Kaggle parallel notebook `format_pass_error()` now shows last 1500 chars instead of `[:200]`; HF_TOKEN setup blurb added to cell 0 header). FIX CODED reply 2026-05-10 ([comment-4415376484](https://github.com/meizhong986/WhisperJAV/issues/321#issuecomment-4415376484)). Ship-notification posted 2026-05-11; awaiting reporter retest. **Note**: open issue #330 (same reporter jasial2, 2026-05-11) reports "(no log captured)" on retest — verify whether the deployed Kaggle notebook actually includes commit 5a0b223. |
| **#320** | Cannot reach PyPI: SSL CERTIFICATE_VERIFY_FAILED (macOS) | Madaerpao | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-05.** Source install on macOS Apple Silicon, **Python 3.12.0a3** (alpha, Dec 2022 build), uv 0.11.8. Preflight check fails reaching PyPI: `<urlopen error [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate>`. FFmpeg 8.1.1, Git 2.50.1 — both OK. The alpha Python is suspicious; standard fix is `Install Certificates.command` from `/Applications/Python 3.12/`. Cluster with #284, #314, #317 (network check failures). **SHIPPED v1.8.14** (commit `e1cddf0` — doc-only: FAQ entry pointing to `Install Certificates.command` + note avoiding alpha Python builds). FIX CODED reply 2026-05-10 ([comment-4415380191](https://github.com/meizhong986/WhisperJAV/issues/320#issuecomment-4415380191)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
| **#319** | [FR] AMD GPU support via DirectML or Vulkan (ggml-vulkan) | kskbl-manbo | `AWAITING REPLY` | **NEW 2026-05-04.** RX 9070 XT (RDNA4), Win 11. Friendly request, offers to test beta builds. **Duplicate of cluster** #114 (DirectML), #142 (AMD ROCm), #239 (AMD GPU). **Responded 2026-05-05** ([comment-4380374766](https://github.com/meizhong986/WhisperJAV/issues/319#issuecomment-4380374766)) — pointed to existing AMD cluster + ROCm-vs-DirectML scope note. |
| **#318** | No option of qwen ASR in the kaggle notebook | jasial2 | `AWAITING REPLY` | **FIXED 2026-05-05** off-release on `main` (commit [4103606](https://github.com/meizhong986/WhisperJAV/commit/4103606)) — `qwen` added to both `pass1_quality` and `pass2_quality` dropdowns. Notebook installs from `git+...@main`, so re-running Step 2 picks up the fix. **Responded** ([comment-4379965185](https://github.com/meizhong986/WhisperJAV/issues/318#issuecomment-4379965185)). |
| **#317** | 安装包如何设置代理进行安装？ (How to set proxy for installer?) | zoqapopita93 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-03.** Chinese network — installer fails to download torch wheel from `download.pytorch.org`. User confirms "通过代理是可以的" (works via proxy) and asks how to configure proxy for the standalone installer. Cluster with #284 (already-AWAITING-REPLY China-network thread) and #314, #320. **SHIPPED v1.8.14** (commit `e1cddf0` — doc-only: FAQ entry documenting `HTTP_PROXY`/`HTTPS_PROXY` env-var setup pattern; pip/uv/git all read these so no separate `--proxy` flag needed). FIX CODED reply 2026-05-10 ([comment-4415377830](https://github.com/meizhong986/WhisperJAV/issues/317#issuecomment-4415377830)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
| **#316** | [FR] User-defined video segmentation feature | bruce609685-collab | `NEEDS FOLLOW-UP` | **NEW 2026-05-02.** **Responded 2026-05-05** ([comment-4380374226](https://github.com/meizhong986/WhisperJAV/issues/316#issuecomment-4380374226)) with ffmpeg manual-split workaround. **2026-05-07 OP clarified** ([comment-4400193978](https://github.com/meizhong986/WhisperJAV/issues/316#issuecomment-4400193978)) — **the original ask was misread**. OP is NOT asking to cut the source video. They observe WhisperJAV already does internal time-based splitting and asks for a way to **mark plot/act boundaries (e.g. "Act 1 / Act 2") that override the automatic segmenter**, so an act doesn't get split mid-segment. This is a different (and simpler) feature than what was proposed. Owner reply needed: acknowledge the corrected understanding, position as v1.9+ scope (joins the configurable-segment-boundary discussion alongside #230 merge module + #322 async). |
...
| **#314** | [Bug] 安装程序卡在 Phase 3 (PyTorch Installation), no progress feedback | zoqapopita93 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **NEW 2026-05-01.** Win 11 + RTX 3060 + CUDA 12.8 install. Console shows `Attempt 1/3` then silent for >2GB download. User can't tell hang vs slow download. Concrete suggestions: (a) pass `-v` or `--progress` to the underlying `uv pip install`, (b) explicit log of the exact command invoked, (c) network-timeout warning for >2GB downloads. Cluster with #284, #317, #320 (install hang / network UX). **SHIPPED v1.8.14** (commit `e1cddf0` — doc-only fix: FAQ "The installer is stuck" entry expanded with Phase 3 progress-checking + tail-the-log guidance. Code-side streaming fix DEFERRED to v1.9.0 install hardening track). FIX CODED reply 2026-05-10 ([comment-4415377477](https://github.com/meizhong986/WhisperJAV/issues/314#issuecomment-4415377477)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
...
| **#312** | VAD Parameters dump_params shows silero values for TEN | TinyRick1489 | `SHIPPED v1.8.14 (notified, awaiting retest)` | **Phase 1 SHIPPED v1.8.13** (commit 90be1a5) — non-ensemble dump_params mirrors runtime firewall. **Responded 2026-05-05** ([comment-4379539708](https://github.com/meizhong986/WhisperJAV/issues/312#issuecomment-4379539708)) with retest invitation. **2026-05-06 TinyRick1489 retest** ([comment-4386005578](https://github.com/meizhong986/WhisperJAV/issues/312#issuecomment-4386005578)) — fix works in non-ensemble mode but `--dump-params` produces near-empty payload in **`--ensemble` mode**: `resolved_config: null`, all `cli_args` null. Both single-pass and two-pass reproduce. **NEW BUG, post v1.8.13 ship.** Hypothesis (UNVERIFIED): the resolver call path for ensemble mode skips the dump_params resolved_config population, only fills mode/sensitivity/subs_language. **SHIPPED v1.8.14** (commit `361ad1c` — extended dump_params block in main.py with `ensemble_config` populated when `args.ensemble`, full pass1+pass2 plumb-through, `_safety_cap_applied` surface). FIX CODED reply 2026-05-10 ([comment-4415376950](https://github.com/meizhong986/WhisperJAV/issues/312#issuecomment-4415376950)). Ship-notification posted 2026-05-11; awaiting reporter retest. |
...
| **#309** | Persistent White Screen in GUI (mklink + WebView2) | Jerry199022 | `NEEDS FOLLOW-UP` | **NEW 04-26.** Detailed bug report with `C:\Users\Jerry` mklink-junctioned to D: drive. Even pure pywebview test fails with white screen → upstream pywebview/WebView2 issue. **Responded 04-30** ([comment-4358678848](https://github.com/meizhong986/WhisperJAV/issues/309#issuecomment-4358678848)) acknowledging upstream boundary. **2026-05-10 Jerry replied** ([comment-4414672013](https://github.com/meizhong986/WhisperJAV/issues/309#issuecomment-4414672013)): a `run.bat` with `WEBVIEW2_USER_DATA_FOLDER=H:\WhisperJAV\webview_data` + `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--no-sandbox` **launches the GUI successfully** — but **NEW sub-bug surfaced**: Save Preset writes to `C:\Users\Jerry\AppData\Roaming\WhisperJAV\presets\1.json` which the mklink junction can't service (preset not found warning). Asks for adjustments to make presets dir configurable / mklink-aware. v1.8.14/v1.9.0 candidate: presets path with env-var override or fallback to `WHISPERJAV_USER_DATA_DIR`. Connects to settings persistence (#96). Cluster with #225, #240. |
...
| **#308** | Add English source + ASS/SSA subtitle format | SangenBR | `NEEDS FOLLOW-UP` (light, ack-only) | **Part 1 SHIPPED in v1.8.13** (commit 7cb4d77). **Responded 2026-05-05** ([comment-4379555973](https://github.com/meizhong986/WhisperJAV/issues/308#issuecomment-4379555973)). **2026-05-06 SangenBR replied** ([comment-4389074332](https://github.com/meizhong986/WhisperJAV/issues/308#issuecomment-4389074332)) with thanks + understanding on ASS/SSA complexity. No new ask. Light-touch close OR keep as v1.9+ ASS/SSA backlog reference. |
...
| **#304** | Why can't I use Fidelity mode? | wenkine-2026 | **CLOSED** 2026-05-05 | **NEW 04-22.** Console shows PyTorch incompat: GTX 1050 Ti (CC 6.1) vs. PyTorch built for sm_75+. Same pattern as #286. CC 6.x hardware isn't supported by the shipped CUDA wheel. **2026-05-04**: reporter wenkine-2026 commented "Please close". **Closed 2026-05-05** ([comment-4380373098](https://github.com/meizhong986/WhisperJAV/issues/304#issuecomment-4380373098)) with brief CC 6.1 hardware-incompat ack. |
...
| **#297** | Blank subtitle file after transcription completed | teijiIshida | `NEEDS DIAGNOSTIC DATA` | **NEW 04-19.** Multi-file queue, only 1 SRT produced. silero-v4.0 fingered as random-fail culprit (his comment on #302). **Responded 04-30** ([comment-4358675722](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4358675722)): v1.8.12 fixes the silero-v4.0 fallback alignment + best_of revert + no_speech tightening. Asked to retest + try whisperseg segmenter. Earlier reply 04-21 superseded. **rev49.9 update (2026-05-11)**: reporter retested on v1.8.12 (silero v6.2, 2h30m file) and v1.8.14 (whisperseg, 2h24m file) — both truncated at 44m output despite different versions/segmenters/files ([comment-4417222949](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4417222949)). v1.8.14 install path confirmed fresh `.exe` (uninstall via Windows Settings → install) ([comment-4420874603](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4420874603)). Owner asked for debug log + balanced-sensitivity A/B ([comment-4418826539](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4418826539), [comment-4421205501](https://github.com/meizhong986/WhisperJAV/issues/297#issuecomment-4421205501)). Reporter committed to retest "tomorrow" with debug logging enabled. **Observations only**: truncation observed across 3 versions (v1.8.11/v1.8.12/v1.8.14), 3 segmenters (defaults/silero-v6.2/whisperseg), 3 file lengths; 44m endpoint in 2 of 3 most recent tests but v1.8.11 test ended at 58m → not a universal fixed cap. **Hypothesis (NOT verified)**: none offered — insufficient data. **Suggested reply addition**: ask reporter to also run a SHORTER test file (30-45 min) to disambiguate fixed-cap-vs-content. **Not in v1.8.14 fix list — do not close.** |
...
| **#280** | Qwen3-ASR TypeError: `check_model_inputs()` | zoqapopita93 | `SHIPPED v1.8.14 (notified, awaiting retest)` | Responded 04-09: upstream `transformers` mismatch, suggested `pip install --no-deps transformers==4.49.0`. **SHIPPED v1.8.14** — defensive `transformers>=4.40.0,<5.0` pin in `pyproject.toml` + `installer/core/registry.py` + FAQ entry in commit `e1cddf0`. FIX CODED reply 2026-05-10 ([comment-4415380555](https://github.com/meizhong986/WhisperJAV/issues/280#issuecomment-4415380555)). Ship-notification posted 2026-05-11; awaiting reporter retest. Full v1.9.0 carryover (Qwen3-ASR fork patch + transformers ≥5.4 upgrade): `memory/project_v1814_cohere_deferred_v19x_carryover.md`. |
...
|---|-------|----------|--------|
| **#328** | [FR] Skip-or-overwrite policy for batch when matching .srt exists | LOW | v1.8.14 (small) or v1.9.x backlog — **NEW 2026-05-09.** unretired1516. Two-flag CLI/GUI knob (`--skip-existing` / `--overwrite-existing`). |
| **#327** | [Suggestion] Add CrispASR (parakeet + WhisperSeg) | MEDIUM | v1.9.x ASR backend expansion track — **NEW 2026-05-09.** rikimtasu. Joins #311 FireRedVAD, #262 Cohere (deferred to v1.9.0), #205 VibeVoice, #232 anime-whisper variants. |
| **#319** | [FR] AMD GPU via DirectML / Vulkan | MEDIUM | v1.9+ — **NEW 2026-05-04.** kskbl-manbo (RX 9070 XT). Duplicate of #114 (DirectML), #142 (ROCm), #239 (AMD GPU). **Responded 2026-05-05** ([comment-4380374766](https://github.com/meizhong986/WhisperJAV/issues/319#issuecomment-4380374766)). Offers to test beta builds — useful tester contact. |
| **#316** | [FR] User-defined video segmentation | LOW | v1.9+/v2.x — **NEW 2026-05-02.** bruce609685-collab. Multi-scene films with unrelated dialogues. Overlaps with v1.9.0 Standalone Merge Module (#230). **Responded 2026-05-05** ([comment-4380374226](https://github.com/meizhong986/WhisperJAV/issues/316#issuecomment-4380374226)) — ffmpeg-split workaround given. |
...
| Date | Changes |
|------|---------|
| **2026-05-11** | **rev49.10. 8 ship-notification replies posted.** All 8 SHIPPED v1.8.14 issues received their "v1.8.14 is out, please retest" follow-up replies. Drafts followed `feedback_github_reply_tone.md` (short, factual, max 2 paragraphs, first person, env-matched upgrade pointer). Status flipped `SHIPPED v1.8.14 (ship-notification pending)` → `SHIPPED v1.8.14 (notified, awaiting retest)`. **Comment URLs**: #321 [4424629639](https://github.com/meizhong986/WhisperJAV/issues/321#issuecomment-4424629639), #324 [4424630845](https://github.com/meizhong986/WhisperJAV/issues/324#issuecomment-4424630845), #325 [4424632467](https://github.com/meizhong986/WhisperJAV/issues/325#issuecomment-4424632467), #312 [4424633722](https://github.com/meizhong986/WhisperJAV/issues/312#issuecomment-4424633722), #314 [4424634696](https://github.com/meizhong986/WhisperJAV/issues/314#issuecomment-4424634696), #317 [4424636393](https://github.com/meizhong986/WhisperJAV/issues/317#issuecomment-4424636393), #320 [4424638836](https://github.com/meizhong986/WhisperJAV/issues/320#issuecomment-4424638836), #280 [4424640213](https://github.com/meizhong986/WhisperJAV/issues/280#issuecomment-4424640213). **Board state**: 7 NEEDS RESPONSE, 4 NEEDS FOLLOW-UP, 1 NEEDS DIAGNOSTIC DATA (#297), ~27 AWAITING REPLY, 8 SHIPPED v1.8.14 (notified, awaiting retest). No code edits, no closures. |
| **2026-05-11** | **rev49.9. v1.8.14 RELEASED — 8 ship-gating fixes flipped to SHIPPED; +2 new issues (#329, #330); #297 moved AWAITING REPLY → NEEDS DIAGNOSTIC DATA.** Open count **78 → 79** (rev49.8 count was likely off-by-one: 78 + 2 new + 0 closures = expected 80, live is 79). Tag `v1.8.14` on `main` commit `f7862f7`; GitHub Release published 2026-05-10T17:27Z UTC (= 2026-05-11 owner local) with `WhisperJAV-1.8.14-Windows-x86_64.exe` (uploaded 05-10T17:25Z) AND `whisperjav-1.8.14-py3-none-any.whl` (uploaded 05-11T12:19Z, ~19h post-publish; rev49.7/49.8-era memory entries claiming "wheel was NOT uploaded" were stale). Pre-release `false`, draft `false`. **8 issues flipped to SHIPPED v1.8.14 (ship-notification pending)**: #324 (commit `3ea442d`), #325 (`f234610`), #321 (`5a0b223`), #312 (`361ad1c`), #314/#317/#320/#280 (`e1cddf0` doc + `#280` also gets defensive `transformers<5.0` pin). All 8 received FIX CODED replies 2026-05-10 (comment URLs in respective table rows). Owner-review-then-post follow-ups still needed per `feedback_draft_review_before_posting.md`. **2 new issues post-rev49.8**: **#329** fzfile 2026-05-11 (Kaggle Pass 1 timeout at 7200s, body `Timeout expired`, 0 replies; same reporter as carry-over #231); **#330** jasial2 2026-05-11 (Kaggle PARALLEL mode, all visible passes FAIL with "(no log captured)", 0 replies; same reporter as the SHIPPED #321 — possibly the same Kaggle issue resurfacing on retest, but only 2 of 4 files explicitly shown in the report). **#297 moved AWAITING REPLY → NEEDS DIAGNOSTIC DATA**: reporter retest 2026-05-11 shows truncation across v1.8.11/v1.8.12/v1.8.14 and across defaults/silero-v6.2/whisperseg segmenters; 44m endpoint in 2 of 3 most recent tests but v1.8.11 test ended at 58m (rules out a universal fixed cap). Reporter confirmed fresh `.exe` install of v1.8.14. Owner asked for debug log + balanced-sensitivity A/B; reporter committed to retest. **Not in v1.8.14 release notes fix list** — do not close. **v1.8.14 section header**: dropped "IN-FLIGHT" framing → "v1.8.14 — RELEASED (2026-05-11)"; collapsed rev49.4 "ChronosJAV: Cohere generator" theme since Cohere shipped DORMANT and is firmly v1.9.0 P0. The rev49.4 LOCKED SCOPE subsection retained but marked "historical, superseded". **v1.9.0 roadmap**: Cohere promoted to "P0 NEW (v1.8.14 deferral)" (joins large-v3 retune, unified segmenter routing, silero retune review). **rev49.8 entry softened**: "eliminating the catastrophic-empty-scene manifestation" → "catastrophe-free in 22/22 runs on owner's hardware; cross-machine validation pending" — same evidence pointer, more honest verb (Rule 0.5 compliance). **New Status Legend entry**: `NEEDS DIAGNOSTIC DATA`. **Board state after rev49.9**: 7 NEEDS RESPONSE, 4 NEEDS FOLLOW-UP, 1 NEEDS DIAGNOSTIC DATA (#297), ~27 AWAITING REPLY, 8 SHIPPED v1.8.14 (ship-notification pending). **No GitHub posts, no closures, no code edits** in rev49.9 — tracker mechanics only. Memory file `MEMORY.md` and `project_v1814_released_resume_pointer.md` to be updated separately to remove stale "wheel NOT uploaded" claim. |
| **2026-05-10** | **rev49.8. v1.8.14 P0 + P1 ship-gating issues RESOLVED in dev - 8 commits + 8 GitHub replies posted.** No additional GitHub state changes (open count unchanged at 78); status flips reflect FIX CODED + ship-notification posted in advance of v1.8.14 release. **P0 (1 issue, 1 commit)**: **#324 micky0526** Korean transcription regression v1.8.10.post3 -> v1.8.13 - root cause confirmed via code reading + the reporter's 5 attached comparison artifacts. The v1.8.11 Fix 2 symbol-only-residue regex (commit 3508005) only covered Hiragana/Katakana/CJK ideographs/fullwidth Latin/ASCII alnum, so Hangul (and any other non-CJK script) was silently dropped as "symbol_only_residue". Fix in commit `3ea442d`: extended `_HAS_LINGUISTIC_CONTENT` regex with Hangul (3 ranges) + Cyrillic + Greek + Arabic + Hebrew + Thai + Devanagari + Latin Extended-A/-B. 11 new regression test cases added to `tests/test_subtitle_sanitizer_symbol_purge.py`; full suite 48/48 PASS. Reply posted ([comment-4415375722](https://github.com/meizhong986/WhisperJAV/issues/324#issuecomment-4415375722)). **P1 (7 issues, 6 commits + 1 doc-only)**: **#325 1Dreamer666** DeepSeek model rename (`deepseek-chat`/`deepseek-reasoner` deprecate 2026-07-24, replaced by `deepseek-v4-flash`/`deepseek-v4-pro`) - fix in commit `f234610`: providers.py default model bumped to `deepseek-v4-flash`, OpenRouter route kept at `deepseek/deepseek-chat` (lags upstream). Reply posted ([comment-4415376078](https://github.com/meizhong986/WhisperJAV/issues/325#issuecomment-4415376078)). **#321 jasial2** Kaggle qwen Pass 1 FAIL on HF Hub unauthenticated warning - root cause was `r['log'][:200]` truncation hiding the real error, not the warning itself. Fix in commit `5a0b223`: Kaggle parallel notebook `format_pass_error()` helper now shows last 1500 chars (where Python tracebacks live), HF_TOKEN setup blurb added to cell 0 markdown header (Kaggle Secrets / Colab sidebar instructions), version bumped 1.8.12 -> 1.8.14. All 4 code cells compile() validated. Reply posted ([comment-4415376484](https://github.com/meizhong986/WhisperJAV/issues/321#issuecomment-4415376484)). **#312 TinyRick1489** `--dump-params` ensemble null payload - fix in commit `361ad1c`: extended dump_params block in main.py with `ensemble_config` populated when `args.ensemble`, including pass1+pass2 full plumb-through and `_safety_cap_applied` surface for the conditional_sensitivity_cap. Verified locally with reporter's exact command shape - dump now populates correctly. Reply posted ([comment-4415376950](https://github.com/meizhong986/WhisperJAV/issues/312#issuecomment-4415376950)). **#314 zoqapopita93** Phase 3 PyTorch silence - doc-only fix in commit `e1cddf0`: FAQ "The installer is stuck" entry expanded with Phase 3 progress-checking guidance + tail-the-log instructions. Code-side streaming fix (Popen line-by-line) DEFERRED to v1.9.0 install hardening track (too invasive for v1.8.14 .exe build cycle). Reply posted ([comment-4415377477](https://github.com/meizhong986/WhisperJAV/issues/314#issuecomment-4415377477)). **#317 zoqapopita93** Installer proxy support - doc-only in commit `e1cddf0`: FAQ entry documenting `HTTP_PROXY`/`HTTPS_PROXY` env-var setup pattern (Windows set/setx + macOS/Linux export), notes that pip/uv/git all read these so no separate `--proxy` flag needed. Reply posted ([comment-4415377830](https://github.com/meizhong986/WhisperJAV/issues/317#issuecomment-4415377830)). **#320 Madaerpao** macOS SSL cert verify failed - doc-only in commit `e1cddf0`: FAQ entry pointing to `/Applications/Python\ 3.12/Install\ Certificates.command` + note avoiding alpha Python builds. Reply posted ([comment-4415380191](https://github.com/meizhong986/WhisperJAV/issues/320#issuecomment-4415380191)). **#280 zoqapopita93** Qwen3-ASR transformers pin - doc-only in commit `e1cddf0`: FAQ entry covering the v1.8.14 defensive `transformers<5.0` pin + downgrade workaround (with env-pip path matched to standalone installer / source / Colab/Kaggle), points to upstream issue #138 + v1.9.0 fork-patch plan. Reply posted ([comment-4415380555](https://github.com/meizhong986/WhisperJAV/issues/280#issuecomment-4415380555)). **No GitHub closures** - all 8 issues stay OPEN with replies; will close after v1.8.14 release + reporter retest confirms. **Items NOT touched this session** (still NEEDS RESPONSE / NEEDS FOLLOW-UP per rev49.7 list): #309 (mklink presets sub-bug), #316 (plot-boundaries corrected reply), #308 (ack-only close), #322 (async-processing Q), #323 (intentional firewall), #326 (info-gather), #327 (CrispASR FR), #328 (.srt skip/overwrite FR). Owner authorized P0+P1 list scope only; remaining items deferred to next review batch. **Branch state**: `dev_v1.8.14`, 14 commits ahead of origin/main since v1.8.13 baseline (5 Cohere arc + 1 cap + 1 retune + 1 update-check + 1 tracker rev49.7 + 4 today + this rev49.8 = 14). NOT pushed per owner's explicit "no pushing" instruction. |
| **2026-05-10** | **rev49.7. Tracker refresh — Cohere DEFERRED, ASR retune + GUI update-check changes committed; +8 NEW issues + 4 status flips since rev49.6.** Open count **70 → 78**. **v1.8.14 theme refresh**: rev49.4's "ChronosJAV: Cohere generator" headline is **DEFERRED to v1.9.0** — all 8 Cohere commits remain on `dev_v1.8.14` but dormant behind GUI `disabled` attribute + `transformers>=4.40.0,<5.0` defensive pin. Rationale: Cohere requires transformers ≥5.4.0 but transformers 5.x breaks bundled Qwen3-ASR fork (`@check_model_inputs()` decorator API change). Upstream issue posted on QwenLM/Qwen3-ASR#138 awaiting maintainer. Full v1.9.0 carryover: `memory/project_v1814_cohere_deferred_v19x_carryover.md`. **Effective v1.8.14 ship-bearing themes**: (a) `conditional_sensitivity_cap` (commit 5e8da48 from rev49.5/49.6 — dormant insurance); (b) **comprehensive FW + Whisper preset retune** across all 3 sensitivities (commit `d0bb379`, 2026-05-09 — engine-split discipline preserved, R1-R5 manual tests passed per owner 2026-05-10); (c) **GUI Check-for-Updates redirect** (commit `ea01a07`, 2026-05-10 — retire in-GUI pip-install path; users sent to GitHub releases page); (d) install UX polish carry-overs (#314, #317, #320, #280 still pending). **+8 NEW issues since 2026-05-05**: **#321** Kaggle qwen HF Hub unauthenticated FAIL (jasial2), **#322** async-processing safety Q on 3070/8GB (kokayin), **#323** WhisperSeg/TEN VAD silero fallback outside `--ensemble` — intentional v1.8.13 firewall (TinyRick1489), **#324** **P0 candidate** Korean transcription regression v1.8.10.post3 → v1.8.13 with `[REMOVED - Symbol Only Residue]` flagging (micky0526), **#325** DeepSeek model-name update (deepseek-v4-flash/deepseek-v4-pro replacing deepseek-chat/reasoner deprecating 2026-07-24, 1Dreamer666), **#326** "1x00 series GPU CUDA errors" sparse log — likely #304/#286 CC 6.1 cluster (techguru0), **#327** Suggestion: add CrispASR (parakeet+WhisperSeg) — v1.9.x backend expansion (rikimtasu), **#328** FR: skip-or-overwrite for batch when matching .srt exists (unretired1516). **0 closed since rev49.6.** **4 status flips AWAITING REPLY → NEEDS FOLLOW-UP**: **#309** Jerry199022 reported (2026-05-10) run.bat workaround launches GUI but Save Preset writes to mklink-junctioned `C:\Users\Jerry\AppData\…\presets\1.json` (NEW sub-bug — connects to #96 settings persistence); **#312** TinyRick1489 reported (2026-05-06) `--dump-params` returns near-empty payload (`resolved_config: null`, all cli_args null) in `--ensemble` mode — NEW bug post v1.8.13 ship; **#316** bruce609685-collab clarified (2026-05-07) original ask was misread — wants user-marked plot/act boundaries that override the auto segmenter, not source-video cutting; **#308** SangenBR replied (2026-05-06) thanks/ack only — light close-or-ack. **Board state**: 11 NEEDS RESPONSE, 5 NEEDS FOLLOW-UP, ~28 AWAITING REPLY (carry-over). **Recommendations for v1.8.14 ship gating** (in `report-only` form to owner): (1) **investigate #324 Korean sanitizer regression URGENTLY** — read attached artifacts, identify which subtitle_sanitizer rule mis-fires on Hangul, possibly v1.8.11 Fix 5a or v1.8.13 polish bundle origin; (2) **#325 DeepSeek model update** is a small, risk-free providers.py edit suitable for v1.8.14 (deprecation date <2 months out); (3) **#321 HF_TOKEN UX** is a small Kaggle notebook doc + actionable error message; (4) consider **#312 dump_params ensemble fix** if low-effort; (5) **#309 mklink presets fallback** is small — env-var override or fallback dir. Defer to v1.9.x: #323 (unified-routing fix), #316 (plot-boundary UI), #327 (CrispASR), #326 (info-gather), #322 (Q&A only), #328 (consider for v1.8.14 if trivial). **No closures, no posts, no code edits beyond the tracker** — owner reviews and authorizes next actions. |
| **2026-05-07** | **rev49.6. v1.8.13 ensemble catastrophe FIX SHIPPED in v1.8.14 dev (commit 5e8da48).** Pattern named **conditional_sensitivity_cap** in `whisperjav/ensemble/safety_caps.py`. When trigger combination is detected (pass1=fidelity + pass2=balanced + sensitivity=aggressive), pass 2 sensitivity is auto-downgraded to `balanced` with console warning. Empirically validated: 22/22 catastrophic-free runs (10 A_fid_bal + 10 E_fid_bal_BAL + 2 F_fid_bal_CON; F suite cancelled at 2 — sufficient signal already). Pass 2 wall-clock now deterministic 62-68s; output count consistent at 47 every run. **Forensic timing finding**: pass 1 fidelity has 4.6x wall-clock variance (190-880s) due to PyTorch cuDNN auto-tune nondeterminism — separate from the catastrophe, output quality unaffected. Documented in `docs/en/faq.md` for end-user expectation; code optimization deferred to v1.9.0+. **Reorder-as-alternative-fix considered + dropped** for v1.8.14 (untested, more invasive); filed as v1.9.0+ research candidate. **Architectural root cause** (CTranslate2 / cuBLAS internal nondeterminism + temperature=0.17 sampling fallback) deferred to v1.9.0+ research. **Files updated**: `whisperjav/ensemble/safety_caps.py` docstring + log message terminology refresh ("Conditional sensitivity cap"); `whisperjav/main.py` integration comment refresh; `docs/en/faq.md` two new entries; `docs/plans/V1814_T142_NONDETERMINISM_INVESTIGATION.md` §18-§20 (validation outcome + open-threads cleanup + reorder discussion). v1.8.14 plan doc cross-reference. Cap is shippable; pivoting now to Cohere feature plan §5 D1-D11 review. |
| **2026-05-07** | **rev49.5. NEW P0 added to v1.8.14 — v1.8.13 ensemble fidelity→balanced catastrophe (non-deterministic).** Owner-reported test (T142) + multi-user reports. Pass 1 = fidelity + pass 2 = balanced (ensemble GUI defaults) intermittently produces 14 entries instead of ~50 on the 293s reference JAV clip. Investigation conducted 2026-05-07 (T142 + T143 + T144_α/β/δ + forensic agent dispatches). **Established facts**: (a) bug is at ASR layer not VAD (whisperseg same in both); (b) bug is non-deterministic (TEST_α rerun produced correct output); (c) bug is not pass-2-balanced-alone (TEST_β passes); (d) bug is not generic cross-engine (TEST_δ with fast/faster pass 1 passes); (e) `safe_cuda_cleanup()` is no-op in subprocess workers (`utils/gpu_utils.py:18-19`); (f) no cuDNN determinism / benchmark settings configured anywhere in WhisperJAV. **Open hypothesis (not verified)**: probabilistic numerical drift in ctranslate2 inference, with fidelity (heaviest pass 1) increasing trigger probability. **Investigation artifacts**: `docs/plans/V1814_T142_NONDETERMINISM_INVESTIGATION.md` (state memo), `tools/ensemble_failure_rate_suite.py` + `tools/ensemble_failure_rate_suite.md` (failure-rate test suite, distributable to other users for cross-machine data). v1.8.14 P0 list updated: now Cohere generator + install UX polish + this catastrophe fix. **Tracker rev only — no GitHub state changes yet.** |

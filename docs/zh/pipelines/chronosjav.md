...
| **Cohere-Transcribe** *(v1.8.14 预览)* | ~4-8 GB | Cohere Labs 2B Conformer ASR。HF 门控模型 — 需要 `HF_TOKEN`。词级时间戳由下游的 Qwen3 ForcedAligner 提供。设置详见 [FAQ](../faq.md#cohere-transcribe预览)。Anime-Whisper 仍是日语调优的默认生成器。 |

!!! tip "建议"
    建议先使用 **anime-whisper** 以获得最佳效果。如果需要更快的处理速度或 GPU 显存有限，可切换至 Kotoba。如果您有 HF 账号并希望在 JAV 内容上对比其他生成器，可尝试可选预览 **Cohere-Transcribe**。
